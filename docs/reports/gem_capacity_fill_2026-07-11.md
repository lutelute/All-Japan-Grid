...
正本: `data/generator_capacity_sources.jsonl`（160→270→**354行**・全行 `capacity_provenance.py verify` PASS）。
第2弾（§7 座標キー回収=名前衝突89件の解消）も同日オーナー承認で実施。
...
- **名前衝突ガードで89件見送り** → **§7の座標キー対応で84件回収済み**（残5=Operating無し4+
  GEMページ内重複行1）
...
## 7. 第2弾: 座標キーapplyで名前衝突89件を回収（同日オーナー承認・+84件 3,559MW）

名前が全国非一意（「〇〇市発電所」等の自動命名）でレコード化見送りになった89件を、
安定キー `plant_key = "geo:<region>:<lon.4f>,<lat.4f>"`（D層Point 4桁丸めと同一書式）で回収した。

**実装**（`scripts/apply_capacity_sources.py` 拡張 + 単体テスト9本）:
- geoレコードは名前索引に入れない（名前が曖昧だからgeoキーにしている）
- ファイル内で同一キーのfeatureが複数（隣接ソーラー分割片等）→ どれにも適用しない
- 名前照合と競合したら: 名前レコードの内包座標（`p03:名前:lat,lon`等）が当該featureを
  確証（±0.005°）しconfidence同等以上なら名前（公式の格を維持）、それ以外はgeoが勝つ
- レコード生成側の自己検証: 全キーがD層plants_allで**ちょうど1 feature**に一致+名前一致
  （84/84パス）。冪等（再適用でmd5不変）

**結果**: 84レコード・3,559MW追記（正本270→354行・verify全PASS）。
plants_all 269→350 / plants_utility 252→333 に適用拡大。

**副次の収穫=既存誤反映の検出と修正（相浦）**: 「相浦火力発電所」は同名2feature
（廃止油火力跡 + P03遺物名が付いた跡地メガソーラー=キューデン・エコソル10MW・裁定が
独立資料で確証済み）があり、従来の名前照合は**廃止0MWレコードをソーラー側にも塗っていた**。
新優先規則でソーラー側はGEM 10MWに正され、油火力側は0MWのまま（正しい）。

**負の結果（採用不能1件）**: 戸畑共同火力の GEM ページは同一テーブル内に別トラッカー由来の
重複ユニット行（`2: 156` と `Unit 2: 156.3` 等）を持ち機械合算が成立しない（1,455.3MWは
二重計上）。推測補正はせず不採用とし、生成側に重複ユニット検出ガードを追加した。
なお第1弾110件は横断検査で重複行なしを確認済み（AUTO検証の検算不一致0とも整合）。
