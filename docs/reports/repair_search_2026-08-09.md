...
> ## ⚠ 2026-08-10 訂正 — 本レポートの結論の一部は誤り
>
> 本レポートは目的関数に**偽電源（合成 slack ＝ 発電機を持たない成分に置く実在しない電源）を
> 数えていなかった**。そのため「現行モデルを全目的で支配する構成がある＝**ここはトレードオフでは
> ない**」と結論したが、偽電源を 4 つ目の目的に加えると次のようになる:
>
> | 島 | 現行を支配する構成（偽電源なし） | **偽電源を数える** |
> |---|---:|---:|
> | hokkaido | 2 件 | **0 件** |
> | east | 4 件 | **0 件** |
> | west | 3 件 | **1 件**（site/off/10MW・285→282本で最大負荷率は同一＝実質的でない） |
> | okinawa | 0 件 | 0 件 |
>
> 接続規則を電圧に見合わせると、孤立ポケットに誤接続されていた発電機が本来の階級のバスへ移り、
> 残されたポケットが合成 slack で賄われる（hokkaido +2 / east +23 / **west +84**）。
> 過負荷の改善（east 603→303本・1,668→823%）と捏造容量の削減（−76GW）は実在するが、
> **無償ではない**。4 目的版は `repair_search_2026-08-10.md` が正。
>
> 以下の本文は当時の 3 目的の記録としてそのまま残す（`feedback_record_failures`）。
