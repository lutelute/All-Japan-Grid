...
次数1で recall@5=6.2% → **次数6+で49.4%**。「ハブは構造で同定できるが通過局は原理的に不可能」
= 手法の失敗でなく対象の性質。Phase 1(関西匿名站)はハブから錨を打つ設計にすべきと確定。
**WLは効かない**(recall@5 18.4→18.9→16.9%)=負の結果として記録。ソース間で近傍の
見え方が違う(builtは下位網あり/keitouzuなし)ため深い集約は差を増幅する側に働く。

**副産物2件（いずれも重要）**:
1. **正典の跨region重複站884組を発見**。all.jsonは10地域ファイルの単純連結(ノード数完全一致)で、
