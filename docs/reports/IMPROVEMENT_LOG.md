...
## 2026-08-09n — **Claude Opus 5** — 既定ON化の後始末: 成果物の再生成と、探索が測り落としていた副作用

**再生成の実スコープは当初想定より狭かった。** 台帳に「要再生成」と書いた3件を精査:

- `dist/sensitivity/` = **要再生成**（唯一の生きた出荷物）。slack は「各成分で最大発電機を
  持つ母線」なので接続規則で参照バスが動く → PTDF は参照依存でずれる
- `docs/reports/pf_frontier_*` = 日付入りスナップショット。08-08 を残し **08-09 を新規生成**
- `uc_to_pf_built` の出力 = 日付入りスナップショット（07-05/07-07）のみ。
  **過去の記録として正しいので上書きしない**（触らないことが正解だった）

**感度行列は物理的内容が変わっていないことを実測で確認した。** east のみ変化
（slack_col 4275→5179）で hokkaido/west/okinawa は **sha256 まで完全一致**（west は
861機46GW繋ぎ替わったのに最大機の母線が動かず不変）。east についても
**PTDF の差は行ごとに定数（行内 std 3.7e-12）= 参照の付け替えのみ**、
**LODF は橋（分母≈0・2,287枝で新旧一致）を除いた2,671万要素で最大差 2.2e-11**。
索引表(bus/branch csv)は1バイトも変わらず。`sensitivity_version` を 1.0.1 に上げ、
第三者が旧 sha と照合したとき理由を追えるよう CHANGELOG に経緯を記録。
※ LODF の初回計測は「最大差 1.18e5」と出たが、これは橋の列（分母ゼロ）を
含めた比較の artifact。**橋を除く**という一手を入れて初めて正しく測れた。

**⚠ 探索が測り落としていた副作用を発見**: 合成 slack（＝実在しない電源）が増える。
hokkaido 30→32・east 268→291・**west 406→490**。電圧に見合わせると孤立ポケットに
誤接続されていた発電機が本来の階級のバスへ移り、残されたポケットは合成 slack で賄われる。
「誤った接続」という捏造を「明示的な偽電源」という捏造に**置き換えた**もので、後者は台帳に
数えられる分だけ正直だが、**捏造量が増えていないとは言えない**。`repair_search` の3目的
（出典なし容量／足した変圧器／超過潮流）は合成 slack を数えていなかった＝**目的関数の欠落**。
次に同種の探索をするときは軸に加えること。最大成分の負荷占有（east 91.6%/west 89.6%）と
需要を持つ成分数は不変なので「解けている範囲」は広がっても狭まってもいない。

**west の AC=FAIL 疑義は解消**（前項で「要確認」としたもの）。`diagnose_pf_frontier`
（バス数上限なし）では **west も AC=OK**。本番 CLI の既定 `--max-ac-buses 7000` に対し
west が 8,213 バスであるための打ち切りで、`project_agj_pf_frontier` との食い違いは無かった。
副次的な改善: east の vm 下限 0.732→0.757・0.95-1.05帯 98.5→98.9%・vm<0.80 が 21→17 バス。
...
一度に一つしか動かさないのは古典的な誤りなので、3 軸の全組み合わせ（接続規則 4 ×
降圧点 2 × 太陽光既定 2 ＝ 16 構成／島 × 4 島 ＝ 64 潮流）を回した
（`scripts/capacity/repair_search.py`、`docs/reports/repair_search_2026-08-09.{md,json}`）。
修復オペレータは what-if 本体の関数をそのまま呼ぶ（実装の二重化を禁じる構造ゲート付き）。

**① 交互作用は実在し、是正の符号を反転させる。**
「太陽光を実測中央値に正す」という同一の是正が、east の最大負荷率を
接続規則 base では **+1,704pt 悪化**、cap では **−772pt 改善**させる。
膨らんだ太陽光は薄く広がった注入なので、除くと発電は実在の火力・原子力へ集中する。
集中先が電圧に見合わないバスなら悪化し、見合っていれば改善する。
**単独測定では、この是正は「やってはいけないこと」に見えていた**（08-09k ①の判断を訂正）。

**② 捏造量を第一級の目的関数にした。** 介入台帳の原則を探索にも適用し、
「過負荷が減ったか」だけで採点しない。3 目的（出典のない発電容量 MW / こちらで足した
変圧器 台 / 超過潮流 MW）の非劣解を重み付けなしで出す。

**③ 結論: トレードオフではなかった。** 現行モデルを**5 指標すべて**で下回る構成が存在する。

| 島 | 構成 | 過負荷 | 最大負荷率 | 超過潮流 | 捏造容量 | 捏造設備 |
|---|---|---:|---:|---:|---:|---:|
| east | 現行 (base/なし/10MW) | 603 | 1,668% | 122,493 MW | 85,670 MW | 0 |
| east | **cap / なし / 0.10MW** | **303** | **823%** | **80,529 MW** | **9,767 MW** | **0** |
| west | 現行 (base/なし/10MW) | 285 | 1,894% | 37,954 MW | 117,750 MW | 0 |
| west | **kvfit / なし / 0.10MW** | **252** | **725%** | **22,448 MW** | **42,401 MW** | **0** |

設備を 1 台も足さず、出典のない容量を east で 89% 減らして、過負荷は半減する。
なお超過潮流のみで採点した最良（east cap/降圧点あり/10MW＝63,121MW）は
**951 台の変圧器を捏造し水増し容量を温存している**ので推奨しない。図では別記号にした。

**④ 繋ぎ替えは外科的。** east は 8,235 機中 **167 機（45,464MW）**が別バスへ、うち接続電圧が
変わったのは **72 機（昇圧 70・31,859MW）**だけ（`gen_attach_diff.py`）。名指しの証拠:
姉崎火力 3,600MW **66→500kV**・川崎火力 3,420MW **66→154kV**・横浜火力 2,800MW **66→154kV**・
東扇島火力 2,000MW **66→154kV**、west は堺港火力 2,000MW **77→500kV**・知多火力 3,966MW 154→275kV。

**⑤ 測定器の誤りを 1 件捕捉。** 負荷率を電力基準（|P|/(max_i_ka×kV×√3×parallel)）でも
毎回計算して pandapower の電流基準と突き合わせたところ、hokkaido で **14.58pt の食い違い**。
原因は**混在電圧線**（110/66kV）— 同じ P なら低圧側の電流が大きく pandapower は
max(i_from,i_to) を採るのに、こちらが from 側の電圧で定格を組んでいた。**両端の低い方**に
直して全 815 本 0.00pt 一致、本計測 64 構成すべてで 0.0pt。モデルではなく器の誤りだった。
`gen_attach_diff` の初版も「繋ぎ替え 37 件」と出したが、これは名前で突合して 8,235 機が
2,930 の一意名に潰れた artifact（真値 167）。**striking な数値 2 件とも公表前に独立検証で捕捉**。

**⑥ 残差が次の仮説を指している。** east 最良構成に残る 370 本のうち 284 本が依然 66kV、
282 本が需要側優勢、放射状端点はわずか 6 本。**真因B（降圧点の欠損）が残差の主因**であり、
これは捏造（変圧器を足す）ではなく OSM／公開系統図からの**出典付き充填**で埋めるべき、
というのが機械の出した次の一手。

**未適用**。採否は人間判断で、採るなら介入台帳に①根拠②帳簿③無効化を登録する。
回帰は `tests/test_repair_search.py`（9 本・混在電圧と並列回線を固定）。
図: `docs/assets/analysis/repair_{pareto,map_east,map_west}_2026-08-09.png`。
教訓（再発）: `nohup … &` を Bash ツールの `run_in_background` の中で使うと子が殺され、
30 分ぶんの探索を JSON 未書き出しのまま失った → 1 構成ごとのチェックポイント＋resume を実装。

## 2026-08-09k — **Claude Fable 5** — 過負荷の診断連鎖が閉じた: 真因は**接続電圧と降圧点の欠損**

前項までで需要は無罪・容量較正では減らないと分かっていた。残る発電側と網側を潰した。

**① 太陽光是正は逆効果（仮説を棄却）** `whatif_solar_default`
既定値を 10→1→0.10MW と振ると最大負荷率は east 1668%→3371%、west 1894%→2439% と
**悪化**する。膨らんだ太陽光は分散電源として過負荷を**隠していた**だけだった。

**② 網の欠損でもない** `overload_vs_topology`
過負荷はメッシュ側に偏り（east 11.4% vs 橋 1.1%）、迂回中央値は 1 ホップ＝並行回線が
すぐ隣にある。介入#22（同名変電所のヤード連結）も橋 2529→2521・過負荷 602→602 で無効。

**③ 真因A: 発電機が電圧を見ずに最寄り変電所へ繋がれている**
east の過負荷 602 本中 521 本（87%）が 66kV、500kV は **0 本**。66kV 線が 1,144〜2,188MW を
流している。`attach_generators` は `min(sub_bus, key=距離)` で繋ぐので、数が桁違いに多い
66kV がほぼ常に最寄りになる。結果 east は発電容量の **53.2%（99GW）が 66kV バス**に載り、
姉崎火力 3,600MW・川崎火力 3,420MW・横浜火力 2,800MW まで 66kV 接続（実系統では 500/275kV）。
`whatif_gen_voltage` で繋ぎ方を 3 通り試した。**外部の接続電圧表は持ち込まず**、
判定基準はモデル自身の導体定数から測った階級の梯子（66kV 137MVA / 154kV 533 /
275kV 1,905 / 500kV 6,928MVA）だけで作る。最良は cap（バスの受電容量で選ぶ）で
east 超過潮流 122,493→90,360MW（-26%）、west 最大負荷率 **1894%→708%**。

**④ 真因B: 降圧点が桁で足りない**
east は 66/77kV バスが 4,727 個あるのに**変圧器は 630 台**。66/77kV 需要のうち降圧点
直結は 20.7% で、42% は 3 ホップ以上先、**10.2%（4,745MW）はどの降圧点からも到達不能**
（west は 24.6%＝13,762MW）。基幹の電力が 66kV の導体を横に何ホップも流れて需要へ届く形に
なっている。`whatif_stepdown` で上位電圧が既にモデル内 10km 以内にある場所だけに降圧変圧器を
足すと（east +951台、west +781台）、east 過負荷 603→422・超過 122,493→79,424MW（-35%）、
west 285→205・37,954→22,538MW（-41%）、最大負荷率 1894%→1258%。変電所は足していない。

**途中の自己修正**: 橋判定で pandapower の `parallel` 列（east 6,122 行中 4,410 行が
2 回線以上）を数えず、2 回線区間を「1 本＝橋」と誤判定していた。橋 41.3%→15.4%、
迂回中央値 8→1 に訂正。座標潰し・エッジ潰しに続く「多重度の潰し」の**四度目**。

いずれも**未適用**。採否は人間判断で、採るなら `docs/MODEL_INTERVENTIONS.md` に
①根拠②帳簿③無効化を登録する。

## 2026-08-09j — **Claude Fable 5** — 潮流のずれの正体: **太陽光の既定容量が実測中央値の100倍**

需要は無罪・容量は較正済みという前段を受け、発電フリートを監査した。

**発見: モデル総容量477GWの48.3%(230GW)が燃料別既定値による合成。**
発電所19,138件中17,858件が既定値で、うち`capacity_mw = -1`の番兵値が3,936件。
**九州(2,549件)と沖縄(32件)は実容量ゼロ＝全件が既定値**。

**核心**: 太陽光の既定値10MWに対し、**実容量が付いた600件の中央値は0.10MW＝100倍の乖離**。
OSMの太陽光ノードは小規模設備が大半なのに一律10MWを与えたため、
モデルの太陽光は180GW＝実績ピーク56.7GWの**318%**に膨張（東京610%・中部309%・九州260%）。
潮流本体はゾーン需要にスケールするので総量は合うが、**注入が「OSMに太陽光ノードが多い場所」へ
過剰に集まる**空間的な歪みが残る。これが潮流のずれの有力な原因。

**途中の訂正**: 当初「太陽光は実績の1割で不足」と書きかけたが、既定値を織り込むと
**逆に3倍の過剰**だった。OSM実容量(6.3GW)だけ見て結論を出すところで、
潮流本体が実際に使う値(既定値込み180GW)を確かめて訂正した。

NAS側も拡張: `aggregate_fuelmix_from_nas.py`で10社の電源種別実績を集約(432ファイル・
528,211行→6KB)。社ごとに形式が全く違い、3回の修正を要した——①ファイル名規約が
バラバラ(202308.csv/year_2024.csv/H29_Q2.csv)→中身で判定 ②中部の説明文
「太陽光の自家消費分は…」をヘッダと誤認→日付列との同時存在で判定 ③中部の別系統
ファイルの合算列「エリア風力・太陽光発電量」を太陽光として拾い9434%の異常値→合算列を除外。
実績は太陽光ピークが需要の20〜58%(九州57.9%・東北54.3%・四国53.1%)。

実装 `scripts/capacity/audit_generation_fleet.py`、ゲート `tests/test_generation_fleet.py` 5件、
正本 `docs/reports/generation_fleet_audit_2026-08-09.md`。直し方は提案のみで未適用。
...
（種別02・30分値・10エリア×19,488断面・出典明記で自由利用）で検証した。

**結果: 需要は無罪。** 水準はOCCTO中央値の×1.43・ピーク合計の×0.80で計画断面として妥当。
**空間配分は構成比のずれが最大1.1ポイント（東北）で他は概ね±0.2ポイント以内**——
県別電力需要実績を重みに使う配分がエリア単位で実績とよく合っている。

したがって残る候補は①発電機の出力配分②網の欠け。**②が疑わしい**——橋が30〜36%を
占めるのは「並行経路が無い」ことそのもので、実系統ならメッシュで分散する潮流が1本に
集中する構造になっている。**欠けている線を埋める方が容量を直すより効く可能性が高い**。
これは[[project-agj-keitouzu]]の裁定キュー92本（断絶30本）に直結する。

実装 `scripts/capacity/validate_demand_vs_occto.py`、正本
`docs/reports/demand_validation_2026-08-09.md`。

## 2026-08-09f — **Claude Fable 5** — 線路容量の理論値を公表値で較正: **約2倍の過大評価**を確定

律速だった島内線路の容量に着手。`data/external/kansai_td/154kv_more_line.csv`（関西送配電の
空容量マッピング・154kV以上235線路）に**設備容量・運用容量・制約要因**が入っていた。
[[reference-utility-data-licensing]]どおり All-Rights-Reserved なので**私的検証にとどめ、
公開は電圧階級ごとの比（無次元）と制約要因の分布のみ**とした。比だけで較正には足りる。

**結果**: モデルの理論値(√3·V·I)は運用容量の **1.87〜2.11倍**。二段に分解できる——
①理論÷設備容量は500kVで1.05とほぼ正しいが低電圧ほど過大(77kVで1.87)＝代表電流値が大きい
②運用÷設備容量は500kVで0.50・275/154kVで0.75＝熱容量いっぱいには流せない。
較正係数は全階級で **0.47〜0.54** に収まり、**単一係数0.5でほぼ足りる**。
制約要因は熱容量77%・同期安定性5%で、熱容量を基礎に置くモデルの作りは妥当と裏づけ。

**最大の含意**: 較正0.5を当てると西日本の過負荷が **2.2%→8.3%**
（`hosting_capacity.py --capacity-factor 0.5` で再現可）。実系統は超過ゼロで運用されるので、
これは**モデルの潮流そのものが実態とずれている**ことを意味する。容量の較正は接続可能量を
「良く」せず、むしろ**潮流側の歪みを可視化する**のが効用だった。
**次に見るべきは需要配分と発電機の出力配分**で、容量データではない。

実装 `scripts/capacity/calibrate_line_capacity.py`、正本
`docs/reports/line_capacity_calibration_2026-08-09.md`。**提案であって未適用**——
出典は関西エリアのみで他エリアへの外挿可否は別途確認が要る。適用は人間判断＋
[[feedback-intervention-registry]]記帳。

## 2026-08-09e — **Claude Fable 5** — 連系線の運用容量を出典付きで確立（律速だった容量データの第一歩）

「NASのデータも使ってよい」指示。NAS(10.0.70.169/189)・pws-160core とも本機からは
到達不可(LAN外)だったため、**ローカルの `data/external/occto/` を精査**したところ、
OCCTO 系統情報公表の種別04（連系線関連・30分値）が15か月分あり、
**運用容量そのものが公表値として入っていた**。OCCTOは出典明記で自由利用。

抽出: 連系線 **14本 × 2方向 = 28レコード**（246,432断面から）。値は既知の事実と整合
（北海道・本州間900MW / 周波数変換設備2,100MW / 相馬双葉幹線 順6,400 / 関門 逆2,850 ほか）。
運用容量は季節・系統状態で変動するため、value は期間中の最大（＝設備能力の下限として
読める）とし、最小・中央値・観測断面数を note に残した。

規約は既存と共通化: `scripts/provenance.py` の ProvenanceSpec に乗せ、
`scripts/interconnector_provenance.py verify` で28件全通過。一次情報のみ許容・
field を運用容量に限定（熱容量と混ざると意味が変わるため）。捏造防止が実際に効くことを
`tests/test_interconnector_provenance.py` 13件で確認（URL欠落・引用空・非数値・
二次情報・field違いをすべて拒否）。

**位置づけ**: これは連系線のみで、島内線路の容量は依然として理論値のまま
（接続可能量算出の律速はそちら）。モデルへの適用は人間判断＋介入台帳記帳が必要。

**続けて対応表を作成**（`map_occto_links_to_model.py`）。名前で突合すると相馬双葉幹線・
本四連系線・三重東近江線などが「モデルに無い」ように見えたが、**地理で裏を取ると設備は
実在**し、枝名が OSM 由来で端点の変電所名から機械生成されているだけだった
（三重東近江線＝`東近江開閉所~三重開閉所線`）。自分の記憶則
（[[feedback-verify-before-claiming]]）どおり第二経路で確かめて誤報を回避。
結果: 名前候補あり5本 / 地理のみ6本 / 集約概念(フェンス)3本。
裁定時の注意として「電圧を必ず見る」を明記（相馬双葉幹線は500kVだが候補上位に275kVが来る）。

## 2026-08-09d — **Claude Fable 5** — 感度行列を出荷物に昇格（Ybus と同じ形）

`scripts/` に閉じていた PTDF/LODF を `dist/sensitivity/` として再利用可能にした。
Ybus v5.0.0 の出荷パターン（版・changelog・sha256・品質ゲート）を踏襲。

**設計判断: 行列本体は git に入れない。** 密行列で east 183MB / west 339MB あり、
再生成は 2〜5 秒で済む。代わりに**索引表（行→枝・列→バス、容量と橋フラグ込み。計1.9MB）と
sha256 指紋**を同梱し、再生成物の同一性を検証できるようにした。索引表が無いと行列を
解釈できない（どの行がどの枝か分からない）ので、そこだけは必ず配る。

索引表の充足: 枝は容量 100%・橋フラグ付き、バスは built ノードIDと座標 100%
（結果を地図に戻せる）。品質ゲート `tests/test_sensitivity_dist.py` 21件——
索引表と行列の形の一致・容量と橋の整合・参照バス列がゼロ・LODF対角が−1。

併せて `dataset/03_sensitivity_screening/` を追加（既存の 01 MATPOWER / 02 UC と同じ体裁）。
注入→潮流・枝停止→潮流・全地点の混雑感度の3例を1スクリプトで示し、west 7,087地点が
93ミリ秒で出る。行列が無ければその場で生成する。dataset/README に「回し方その3」として
節を追加（以降の節番号を繰り下げ）、限界の節に容量が理論値である旨を追記。
実行時に気づいた点: 参照バスの列は定義上ゼロなので「繋ぎやすい地点」1位に紛れる。
比較から除外した。

## 2026-08-09c — **Claude Fable 5** — 自己指摘の構造的対策を実行: 座標解決を正典実装に一本化

同一セッションで3ファイルが同じバグ（座標→単一IDの後勝ちでノードが静かに脱落）を
踏んだため、`src/topology/coords.py` に `CoordIndex` を新設して集約した。

**監査**: 座標キーを扱う17ファイルを調べ、**同じ罠は3ファイルのみ**と確定
（他は出力整形か `national.py` の意図的な格子集約）。移行対象は
`crosscheck_keitouzu.py` / `gen_ybus_from_db.py` / `toporag/vectorize_substations.py`。

**回帰確認**: 3ファイルとも移行前後で出力が完全一致（crosscheck の辺分類
386/294/92/689/26、Ybusスパイの一意ペア27,913、toporag の n_live 6434・AUC 0.888・
層別recall全5値）。`tests/test_topology_coords.py` 6件でトラップを固定（多層変電所の
座標共有・跨region重複・電圧一致優先・実データで n_would_be_lost>3000 の前提確認）。

実データでの規模: 17,336ノードが13,127座標に載り、**潰せば4,209ノードが消える**。

**併せて非決定性を1件解消**: toporag の BFS が集合を走査順のまま辿るため、
`deg_kv` に記録される「最初の辺の電圧」が実行ごとに揺れ、構造類型のクラスタ割当が
run 間で変わっていた（移行とは無関係の既存問題。主要指標 AUC/recall は影響なし）。
近傍を `sorted()` して固定し、2回連続実行でクラスタ規模まで完全一致を確認。

## 2026-08-09b — **Claude Fable 5** — 感度行列の使い道を実物で示す: 地点別の系統混雑を全点一括算出

オーナー指摘「凄さとか何に役立つとかよくわからない」を受け、速度の数字でなく**成果物**で示す。
`scripts/sensitivity/hosting_capacity.py`: 地点 b に P MW 注入したときの枝 j の潮流変化は
PTDF[j,b]·P なので、**全地点の系統影響が行列の列演算だけで出る**。

実測: west 7,086地点を **0.47秒**、east 5,392地点を 0.25秒。同じ答えを潮流の解き直しで
得るなら 7,087回 × AC 42秒 = **3.4日**相当。主指標は「1GW繋いだときの最悪混雑増分 [%/GW]」で、
east は良い地点 59 / 悪い地点 379 %/GW と**6倍の差**。どこに繋ぐかで系統影響が桁で変わることを
地図で可視化した(`docs/assets/sensitivity/hosting_capacity_2026-08-09.png`)。

**設計上の判断2件**: ①絶対値の「接続可能量[MW]」は基準ケースに1本でも過負荷枝があると
**全地点0**になり答えにならない(本モデルは実際そうなった: east 175本・west 77本が既に容量超過)。
基準の実行可能性に依らない**相対指標**を主軸にした。②制約は154kV以上に限定。過負荷は66kV層に
集中しており(east 4,471本中520本)、下位系を制約に入れると全地点が塞がる。沖縄は最高132kVのため
このしきい値では対象外。**次の律速は計算ではなく容量データ**であることが浮き彫りになった。

## 2026-08-09 — **Claude Fable 5** — 感度行列(PTDF/LODF)で潮流を高速化し精度を実測（オーナー依頼）

`scripts/sensitivity/` を新設。PTDFを最大連結成分から構築し、潮流を**行列ベクトル積1回**に置換。

**速度**: 1断面 AC 89ms(hokkaido)〜63.8s(east) → PTDF 0.008〜4.1ms。**×1,434〜15,488**。
構築コストの回収点は hokkaido 0.1断面 / east 0.03断面 = 実質2断面目から得。
**精度**: 枝潮流の誤差 中央値 0.2〜0.6MW・p95 4.1〜48MW(平均潮流28〜111MW)。
**中央値は小さく裾が重い**形。大潮流枝(上位10%)の相対誤差p95が実務指標。screeningには十分だが確定値はAC必須。
**N-1**: LODF一発 vs 解き直しが**全島で不一致0MW=DCの枠内では厳密**、×18〜2,148。
ただし**橋(落とすと網が割れる枝)が30〜36%**あり、そこはLODF不定で個別に解く必要がある
(橋の多さ自体が網の弱さの指標)。

**踏んだ穴3件(いずれも実装に恒久化)**:
①`rundcpp`はppcを作り直すので`ppc[BR_STATUS]`直書きの停止は無効 → 実要素の`in_service`を落とす
(修正前は612MW食い違い)。②橋の判定を`~isfinite(lodf)`でやると誤検出201本・真の橋を取りこぼす
→ LODFの分母そのもの(自己感度PTDF[k,f]−PTDF[k,t]が1)で判定(修正前15.6MW→0MW)。
③**主成分をslack1枚で解くとeast/westはnr/iwamoto/bfsw/distributed全滅**。本番は成分ごとの
合成slack+角度しきい値のpruneはしごで収束させている → 精度の基準を本番`solve_island`のAC解に変更。
これは「本番の収束がその手順込みで成立している」ことの実証でもある。

## 2026-08-08 — **Claude Fable 5** — 潮流の到達範囲を実測: 全4島15,115バスでAC収束（オーナー依頼「どこまで計算できるか」）

`scripts/diagnose_pf_frontier.py` を新設し、潮流本体と**同一の build_island_net + 無効電力補償**で
収束・電圧品質・成分構造を一括計測（本体と手順が1つでも違うと west が非収束・沖縄の電圧帯も崩れる
＝忠実性が必須と実証）。

**解けるか**: 全4島 DC/AC とも収束（計93秒）。hokkaido 802 / east 6002 / west 8213 / okinawa 98 = **15,115バス**。
正常帯(0.95-1.05)は okinawa 100% / west 99.1% / east 98.5% / hokkaido 85.9%。崩壊域(<0.80)は全国42バス。
**west の既定FAILは数値的失敗でなく `--max-ac-buses 7000` による方針スキップ**（8213>7000）。
上限を外すと108秒で収束し正常帯99.1%。06-27の「west AC収束=断片化の見せかけ」は
介入#21/#22(dedup・サイト変圧器)既定化後の現モデルでは当てはまらない。**既定値の見直しはオーナー判断**
（方針変更につき MODEL_INTERVENTIONS 記帳が要る）。

**意味があるか**: 最大連結成分が負荷の **hokkaido 93.7% / east 91.6% / west 89.6% / okinawa 93.0%** を保持。
成分数(35/312/541/6)は多いが実体は小断片の裾。**残り約10%の需要は孤立断片に載り合成slack=実在しない
電源が供給**しており、これが現時点の計算限界そのもの。

**副産物**: `gen_ybus_from_db.py`(スパイ図生成)に本日と同種の座標後勝ち欠陥を発見・修正。
**変圧器リンク2180本が ia==ib で全滅**しており公表図が実際のYbusより疎に見えていた
(非対角ペア 16858→27913)。数値Ybus(`gen_ybus_numeric.py`)は潮流と同一モデルを使うため無傷。

## 2026-08-08 — **Claude Fable 5** — topoRAG Phase 0 実証 + 突合バグ発見・訂正（オーナー着想）

オーナー着想「回路のネットリスト類似度(topoRAG)をこのプロジェクトに適用したら」を実装・実証。
変電所エゴグラフを35次元(電圧階級1/0・階級別次数・層数・隣接局素性)にベクトル化しコサイン比較。

**結果**: 名前も座標も使わず、検証済みcrosswalk対応(正例640)と誤マッチ確定(負例16)を
**AUC=0.888**で判別。ランダム対照0.150に対し正例中央値0.919で空間は縮退していない。
ただし**検索は全体recall@1=7.3%と弱い**——が、構造の豊かさで層別すると単調に効く:
次数1で recall@5=6.2% → **次数6+で49.4%**。「ハブは構造で同定できるが通過変電所は原理的に不可能」
= 手法の失敗でなく対象の性質。Phase 1(関西の匿名変電所)はハブから錨を打つ設計にすべきと確定。
**WLは効かない**(recall@5 18.4→18.9→16.9%)=負の結果として記録。ソース間で近傍の
見え方が違う(builtは下位網あり/keitouzuなし)ため深い集約は差を増幅する側に働く。

**副産物2件（いずれも重要）**:
1. **正典の跨region重複変電所884組を発見**。all.jsonは10地域ファイルの単純連結(ノード数完全一致)で、
   OSM抽出bboxが境界で重なり同一設備が二重計上。chubu↔hokuriku 227・chugoku↔shikoku 188など。
   例: 高千帆変電所220kV は18m離れた並行部分グラフを両region分持つ。全国集計/潮流/島判定への
   影響は要検討(正典側の課題)。
   **オーナー反証「電圧階級の違う変圧を伴っているだけでは？」を検定(同日)**: 対の電圧層集合は
   **811組が完全一致**(変圧の対なら互いに素になるはず)、互いに素の69組も54組は片側が電圧未記入
   =属性欠落。→変電所の二重計上は確定。**ただしエッジ側は指摘が的中**: 同一端点エッジ群2367の
   うち**623は異電圧=並行回線として正当**、同電圧同線名の**1164が真の二重取得**。
   「同一端点エッジ=重複」と数えるのは過大だった。この区別を検証項目として実装に恒久化。
2. **自分の突合コードのバグを発見・修正**: coord2idが1座標1IDで、610変電所が隣接グラフから
   脱落していた。修正後 **整合率83.5%→88.1%・食い違い127→92・断絶69→30**(前報を訂正)。
   同一座標=同一物理サイトとして層/重複を結線する実装に変更。

実装: `scripts/toporag/{vectorize_substations,analyze_similarity}.py`・
`docs/reports/toporag_phase0_2026-08-08.{md,json}`・`docs/assets/toporag/phase0_2026-08-08.png`。
教訓(再発): 変数シャドーイング2件目(`km`がKMeansモデルを潰した)。実行の決定性は2回実行で確認済。
...
（潮見×2・平田—郡山7.8deg・中越—小国町9.1deg・沖縄番号変電所×5など）→裁定時の優先確認対象。
...
002/003/004は先端がmain祖先=取り込み済み。001は不採用(核心のPF収束はv1.4.0で別解済み・
容量不明→10MW既定値が捏造禁止方針に抵触・実測4ファイル衝突)。005は機能実装済み+
コミットobject欠落でマージ不能。掃除(worktree/ブランチ削除)は破壊的につきオーナー承認待ち。

## 2026-07-12(未明) — **Claude Fable 5** — 1-C第2弾: 座標キーapplyで名前衝突84件・3.6GW回収+相浦の既存誤反映修正

オーナー承認（「2，1の順でやっていい」）。詳細=`gem_capacity_fill_2026-07-11.md` §7。

- `plant_key="geo:<region>:<lon.4f>,<lat.4f>"` を新設し applyを2経路化（geoは名前索引に不混入・
  ファイル内キー重複は不適用・競合時は内包座標で確証された名前レコードのみ優先）。単体テスト9本
- **84レコード3,559MW追記**（正本270→**354行**・verify全PASS・plants_all 269→350適用）。
  生成側自己検証=全キーがD層ちょうど1 feature+名前一致
- **相浦の既存誤反映を検出・修正**: 同名2feature（廃止油火力跡+P03遺物名のメガソーラー）に
  廃止0MWが両塗りされていた→ソーラー側をGEM 10MW（裁定が独立資料で確証）に是正
- 負の結果: 戸畑共同火力=GEMページ内の別トラッカー重複行で合算不能→不採用+検出ガード追加
- ゲート: 1,196 passed（+9）・既知3失敗のみ

## 2026-07-11(夜) — **Claude Fable 5** — 1-C GEM容量充填: 九州/沖縄110件・18.9GWを出典レコード化

詳細=`gem_capacity_fill_2026-07-11.md`。gem.wiki 11,631ページ機械収穫→決定的突合
（AUTO134/AMBIG1901/major未マッチ103）→エージェント316体の裁定+敵対的検証（2ラン合算）。

- **確定202**（auto 132/裁定67/捜索3）→ **出典レコード110件（18,914 MW）**を正本に追記
  （160→270行・verify全PASS）。名前衝突ガード89件は誠実に見送り（applyが名前照合のため）
- 検証の実効: 滝上（出光バイナリー混在の過大計上）・大分共同（同一構内別発電所）をAUTO反証で排除。
  容量検算不一致0。裁定1,901の95%はnull=密集FIT太陽光の同定不能を捏造しない
- 障害復旧: 月次上限で初回ラン148体失敗→セッション死→scratchpad消失。transcriptからスクリプト
  逐語回収+journalから1,849裁定回収+再収穫**ドリフトゼロ**確認で完全復旧（継続118体・エラー0）
- ゲート: 1187 passed（既知3失敗=okinawa supplement混入・本変更と無関係）。NOTICE にGEM CC BY 4.0帰属
...
詳細=`east_voltage_refinement_2026-07-10.md` §3d/3e・`transformer_existing_2026-07-04.md` §6。

- **what-if感度で「定格律速」確定**（`a56ff61`）: 仮定定格450MVA+上北沢仮リンクで城南チェーン
  vm 0.73→0.90-0.95・east低電圧23→0（what-if値は介入に使わない）。hourly-shunts の24h全時刻
  証拠・`gen_ybus_numeric` への #22/#23 配線も同コミット
- **銘板第4弾（城南8サイト・22行 → 総633行/invalid 0）**: 池上275/66 **300MVA×3** を一次資料で
  確証 → 構造DB@nameplate（tokyo 3→4器）。**PF実効はゼロ** = builtの池上が66kV単独ノード
  （275kV地中系のOSM欠落）でtrafo未生成（V4プローブで解の完全同一を確認）。
  練馬/和田堀のsn_mvaは未取得=負の結果。大井ふ頭はOSMにサイト自体が無い
- 含意: 城南解消は「練馬/和田堀のsn一次資料」×「給電構造の連結」の両輪。銘板単独では動かない
- ゲート52 PASS・全域structures再生成 err=0・正典PF/Ybusは不変（銘板は1器もPF未結線のため）

## 2026-07-10(午後) — **Claude Fable 5** — east電圧精緻化: 大間ポケット解消(#23)・城南根因確定

オーナー指示「精緻化してみて」。外れ値35バスを構造診断で**2クラスタに完全分解**し、
片方を出典付きで解消、片方は根因確定まで。詳細=`east_voltage_refinement_2026-07-10.md`。

- **介入#23 未供用線の正直化**(`--deenergize-unbuilt`): 大間幹線(500kV61km・大間原発
  運転開始未定=J-POWER一次)を出典必須リストで in_service=False → **east過電圧12→0**
  (vm_max 1.748→1.023)。idle原子炉の正直化と同型・供用開始で行削除
- **介入#22 サイト内変圧器リンク**(`--site-trafos`): 同名変電所+0.6kmの異階級連結。
  保守版は効果小(east+6/west+20)・**グレーゾーン54件**(加賀500/275等)を候補化=人間レビュー待ち
- **#20精緻化 時刻別シャント**(`--hourly-shunts`): 24h経路のbase固定張り→軽負荷過補償
  (t=3 vm2.99)を時刻追従で解消 → **vm_max 2.99→1.77・vm_min 0.748→0.859・損失-500MW**
- **城南チェーン23バスの根因確定**: 給電trafo未連結(上北沢1.41km)+典型定格過小(練馬/和田堀
  ≈100-130MVA)+OSM 66kV-onlyサイト。正道=東電PG有報の銘板収集(Phase-B型)・捏造しない
- 全レバーopt-in(正典不変)・served不変(0.9801)・ゲート52 PASS
...
コミット: `522ac81`(第1弾)・`2966ce0`(第2弾)・`cda155a`(site_key正規化)。

- **体系ソース発掘**: 有価証券報告書「主要変電設備」表(法定開示・全社統一様式)を10社+J-POWERで実証→sn_total_mva 97件(上北3710/東九州4500/新生駒5600MVA等)。北陸(整備計画「該当なし」)も有報で9サイト取得
- **増強前値**: 整備計画「A→B」の左辺=現有設備として15サイト。安全規則=「着工準備中」限定(東栄の実証: 工事中は旧設備除却済の中間状態がありうる=整備計画800→3,000に対し有報実測1,500)
- **rating系資料の峻別**: 予想潮流一覧(TEPCO/関西・転載禁止)は容量不採用(MW設備容量=銘板でない+All-Rights-Reserved方針)、バンク構成のみ108件(新生駒500/275×2+500/154×4+275/77×2等が初の第一級データ化)
- **保守的削除**: 増強前値由来n_units 11件(整備計画の台数欄=工事対象数であり全体数の確証なし)
- **名寄せ同定4件**: 大井町=北松江・卒古沢=上北・潤井戸=房総・久見崎町=川内原発開閉所。OSM欠損2件(東栄500kV基幹!・高砂)。内房=154/66確定(worklistのkv誤推定)
- **site_key正規化**(cda155a): NFKC+空白除去で「新生駒 変電所」型の表記ゆれを照合側で吸収(意図せぬ統合0件を全域検証)
- **PCクラッシュ復旧**: /private/tmp消失→正典無傷・コンテキスト保持分は復元・消失分は再収集(教訓=中間成果物も早めに正典へ)
- verify 602(existing 257/planned 345)/invalid 0・ゲート21 passed

## 2026-07-04 — **Claude Fable 5** — Phase B: 整備計画PDF機械スイープ(9社)で変圧器出典DB 31→348レコード

前回(07-02)開通した変圧器出典必須DBの体系ソース第1経路「各社整備計画PDF」を全国展開。
詳細 = `docs/reports/transformer_sweep_2026-07-04.md`。

- **9社スイープ**: 並列サブエージェント8体+直接読取(東京)。TEPCOはWebFetch 403→curl+ブラウザUAで突破。採用92行→**317レコード(83サイト・全planned)**。北陸は【主要変電所】欄が明示「該当なし」=正直な0
- **三重確認**: バリデータ機械拒否(reject 0)+原文転記+独立spot check 2件(中部・東栄 800→3,000MVA / 関西・信貴 750×1増設=既設existing 750×3と整合)完全一致
- **峻別の規則化**: 台数明示様式=sn_mva(1台あたり)/台数列なし様式(中部・九州)=sn_total_mva。hv_kv/lv_kv 169件併記(将来のexisting昇格時の適用先指定)。万V/万kVA様式(中国・九州)は×10換算をnote明記
- **planned隔離の実地検証**: 構造DB再生成でsummary完全不変(existingのみ適用の設計が機能)。verify 348/invalid 0・ゲート18 passed
- **負の結果の記録**: 廃止・減設18行(西濃/中信/湯船変電所廃止ほか)+HVDC/FC 4件をレポート§4に将来除却情報として記録(レコード化はせず)
- **次**: worklist既設500kV 206件の充填は13/206→既設個別収集(技報/プレス)が主戦場。Ybus v4はexisting充填と合流
