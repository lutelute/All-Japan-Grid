...
## 2026-09-22 — **Claude Opus 5** — 【正典適用・オーナー「やって良い」】併架線の回線数の是正を正典へ(介入#45 候補)・南海トラフの崩壊の主役が入れ替わる

- **正典はビルダーから一発再生成できない**と判明: `docs/data/built/all.json` は介入 #21/#23/#31/#38/#44 などをスクリプトでその場に積み上げて作られている(ワンコマンド再生成は ROADMAP の将来目標)。ビルダーから作り直すとそれらが消えるので、**同じ作法の介入スクリプト**として当てた
- **【新規】`scripts/apply_mixed_voltage_circuits.py`**(ドライラン既定・`--write` で `.pre_mixedcirc.bak`・帳簿 `docs/reports/mixed_voltage_circuits_2026-09-22.json`): 証拠は全地域 `data/{region}_lines.geojson` の**単一電圧・単一名・circuits タグつき**way。**合成名(`東北電力ネットワーク 66.0kV線` 1,684 way・`A変電所~B変電所線`)は除外**、**本当に併架(枝名の別の線が別電圧)の枝だけ**対象、kv ≤ 0 は触らない
- **証拠規則を多数決から最大値に変更**(ビルダー側も): 線路の回線数は区間の最大であって多数決ではない。富津火力線 500 kV は 1 回線 4 way・2 回線 3 way で、多数決だと **1** になってしまう(最大値なら 2)。1 回線の線路に 2 回線の区間は現れないので、最大値なら**過小評価=偽の過負荷を作らない**
- **結果**: 枝 19,944 本のうち **240 件**(減らす 181・増やす 59)。回線数の総和 36,719 → 36,388(**−331**)。ノード・枝の数は不変。500 kV −12 / 275 kV −90 / 154 kV −230 / 77 kV −10 / **66 kV +22**
- **公表値との突き合わせ**: #44(増やす方向のみ)を再実行すると「OSM 推定が公表値を下回った枝」だけが戻る → **240 件中 4 枝だけ**(66 kV 2・220 kV 2・すべて 1→2)。公表値を優先して適用。**OSM の証拠は公表資料とほぼ一致**
- **下流(南海トラフ)**: `extract_grid_case.py` で再抽出 → 東の枝 7,144 本中 **208 本**の回線数が変わり容量合計 **3,514 → 3,343 GW(−4.9%)**(富津火力線/上総線 27.7→6.9 GW・鹿島線/船津線 13.3→1.9 GW・香取線/湖南線 5.7→1.9 GW)。**run_v10**: 東=崩壊 18→**22**/200 で内訳が **直後 3→11・後 15→11** と反転、受電率 10 分 87.0→84.5%・3 時間 73.0→71.5%、過負荷遮断 2.62→2.88 本。**西はほぼ不変**(崩壊 8/200・10 分 67.2%・3 時間 63.3→63.4%・過負荷 4.80→4.75)
- **予告どおり崩壊の主役が「津波の後」から「揺れの直後」に入れ替わった**。局所是正(`sens_circuits`: 21 件・直後 10・後 11・10 分 84.8%)と一致し、端から端まで一貫
- **副作用(開示)**: #44 の再実行が `all.json.pre_circuits.bak`(2026-09-03 の別アクターの作業中バックアップ・git 管理外)を**上書き**した(スクリプトが無条件に書くため)。9/3 時点の正典は git 686d79b に、本介入の直前は `all.json.pre_mixedcirc.bak` にある
- **未了**: Ybus → MATPOWER の連鎖再生成(正典の派生物はまだ旧回線数)/ 過負荷 KPI の実測 / MODEL_INTERVENTIONS.md への #45 登録(別アクターの未コミット編集のため保留)
- テスト `tests/test_mixed_voltage_circuits.py` 8 件 + ビルダー関連 60 件 passed

---

## 2026-09-21(続 2) — **Claude Opus 5** — 【源泉修正・オーナー指示】併架線の回線数を線路名ごとの証拠で電圧クラスに配る

- **オーナー指示**「源泉で電圧ごとに回線数を分けるのをやって」。`src/powerflow/snapped_topology.py`(ビルダー)を修正。レポート `docs/reports/mixed_voltage_circuits_builder_2026-09-21.md`
- **旧実装**: `circ_eff = circ if (len(build_classes) == 1 or bkv == kv) else 1` = 併架線のタグ合計回線数を**主クラス(最大電圧)が全部持ち、展開した下位クラスは 1 回線**。コメントに「下位を 1 にすると 184% 過負荷(関西 国府支線;岩中国府線 77;33 circuits=3)」とあり、**「合計を残す」か「1 にする」かの二択**になっていた
- **新実装**(`mixed_voltage_circuits_by_name=True` 既定有効): 同じ線路の**単一電圧・単一名・circuits タグつき**の way から `{(線路名, kv): 回線数}` の証拠表を作り(**2 本以上の way の支持が必要**・値が割れたら最頻値・同数なら大きい方)、**タグの合計回線数を保存したまま**電圧クラスへ配る(証拠のあるクラスはその値・残りを証拠の無いクラスで等分・証拠が合計を超えるときは配分せず従来どおり)。`_voltage_class_list`(順序つき)・`_line_name_list`(`/` と `;`)を追加。OSM は電圧と線路名を同じ順で並べる慣習を使う
- **懸念だった関西の事例は解消**: `国府支線` は 1 ではなく **3 → 2**、残り 1 が 33 kV 側へ。合計は保存
- **結果(全 10 地域)**: 修正 **408 件**(上位を減らす 227・下位を増やす 181)。回線数合計 500 kV 1,070→1,052(−1.7%)・275 kV 2,385→2,275(−4.6%)・154 kV 6,142→5,827(−5.1%)・77 kV 4,699→4,671(−0.6%)・**66 kV 15,927→16,085(+1.0%)**。**線路の本数は不変**(トポロジーは動かない)。**東京に偏る(296/408)**=東京領域の OSM は 7,988 本中 322 本(4.0%)が電圧併記。四国 0 件
- **鹿島の回廊**: 香取線 275 kV 6→2、鹿島線 275 kV 8/4→2、湖南線・船津線・取手線 66 kV 1→2。**東京電力 潮流実績の線路名(香取線1･2L・鹿島線1･2L = 2 回線)と一致**
- **3 点セット**: ①根拠= OSM 単一電圧 way の circuits(2 本以上)+ 潮流実績の線路名での検算 ②帳簿= `net.mixed_voltage_circuit_fixes` と `metadata["mixed_voltage_circuit_fixes"]` ③無効化= `mixed_voltage_circuits_by_name=False`。**MODEL_INTERVENTIONS.md 登録は別アクターの未コミット編集のため保留**
- **未了**: **正典 `docs/data/built/` は再生成していない**(下流の Ybus・MATPOWER・pages・ハザードモデルは旧の回線数のまま。再生成はオーナー判断)/ 過負荷 KPI の前後比較は未実測 / 名前と電圧の順番の対応は OSM の慣習に依存(逆順は未検出)
- テスト `tests/test_mixed_voltage_circuits.py` 7 件 + ビルダー関連 60 件 passed

---

## 2026-09-21(続) — **Claude Opus 5** — 同じ鉄塔の別電圧を上位電圧で数えていた(モデル全体の容量過大)・東の崩壊の主役が入れ替わる

- **発端**: 鹿島の過負荷を追う過程。前エントリの「真因は 500 kV の欠落」は**誤りで訂正済**(OSM 生データと open-keitouzu の 2 出典で否定)
- **【欠陥】OSM の `voltage=275000;66000` `circuits=6` を、モデルは 275 kV 6 回線として扱う**(実際は 275 kV 2 回線 + 66 kV 4 回線)。**独立に 7 例を OSM の単一名 way で検算し 6 例で確認**: 富津火力線/上総線 500 kV 8 回線 27.7 GW(上総線は 66 kV・**4 倍**)、新座線/狭山線/脇田線 6 回線(狭山線 154 kV・脇田線 66 kV・3 倍)、秦浜線/下沖線(3 倍)、香取線/湖南線(3 倍)、新栃木線/桐生機械線(2 倍)、上尾線/東埼玉線(2 倍)。下郷線/塩原線はどちらも 500 kV で問題なし
- **規模(上限側の粗い見積もり・測定値ではない)**: 東は線路 6,467 本のうち名前に「/」を含む枝 **603 本**・並列 4 以上 **388 本**で割り当て容量 347 GW、2 回線に直すと 155 GW(差 193 GW)。西は 68 本・7 本・5→3 GW。**東西で 100 倍近い差があり理由は未確認**。OSM 側は東京領域 7,988 本中 322 本(4.0%)が電圧を複数併記
- **【新規】`branch_parallel.value=[{f,t,parallel}]`**(既存の枝の回線数を直す・x は反比例・容量は比例)と帳簿 `output/<run>/<島>/branch_parallel_ledger.csv`。あわせて `branch_removals`(`branch_additions` の**後**に実行)
- **【切り分け】2 つの欠陥が逆向きに効いていた**(東 200 サンプル・同じ乱数): run_v9 は 崩壊 18(直後 3・後 15)。**回線数だけ**直すと 21(直後 **10**・後 11)、**張り直し**も入れると 14(直後 10・後 **4**)。**後で崩れる型は鹿島のトポロジーの作りもの、揺れの直後の型は容量の数え過ぎに隠されていた**。両方直すと**主役が「津波の後」から「揺れの直後」に入れ替わる**
- **デッキへの影響**: v9・v10 の「後で崩れる 17 例のうち 11 例は鹿島の切り離しが引き金」はこの但し書きなしに使えない
- **残る過小評価**: 張り直し版の香取線・鹿島線は計 3,810 MW だが公表の運用容量は約 4,800 MW(2 割低い)。次は運用容量を使う(生値は git 外・帳簿は output/ のみ)
- レポート `docs/reports/nankai_hazard_2026-09-13/dynamics/mixed_voltage_circuits_2026-09-21.md`。**この数え過ぎは [[project-agj-capacity-calibration]]「線路の理論値は運用容量の約 2 倍」の一部を説明しうる(未検証)**

---

## 2026-09-21 — **Claude Opus 5** — 南海トラフ: 17 万 V 以上の変電所の津波停止を公表被災箇所数に較正(run_v9 既定)・候補デッキ v10

- **較正**(`substation.tsunami.protected_kv_min: 170` / `protected_factor: 0.22`・**既定有効**): モデルは 17 万 V 以上を 1 サンプル平均 **22.6 箇所**止めており(東京 5.68・中部 5.40・関西 3.22・四国 1.90・九州 6.44)、送配電網協議会「南海トラフ巨大地震における電気設備の耐性評価」(2026-01)の公表 **5 箇所**(東京 0・中部 1・関西 3・四国 1)の 4.5 倍。合計を合わせる係数 5/22.6 = 0.22 を掛ける。**関西(3.22 対 3)・四国(1.90 対 1)はほぼ合い、東京・九州が外れる**残差は消していない(A40 は県公表の包絡で内閣府の最過酷ケースより浸水域が広く、係数はハザード側の差も吸収)
- **帳簿**: `output/<run>/<島>/tsunami_site_ledger.csv`(サイト名・エリア・電圧・浸水深ランク・補正前後の停止確率)。東 14 サイト・西 27 サイト。**無効化**: `--set 'fragility_overrides={"substation": {"tsunami": {"protected_kv_min": null}}}'`
- **run_v9 の結果**: 東=全域崩壊 20→**18**/200、3 時間受電率 69.7→**73.0%**(2 時間 72.2→76.3%)、95 分の鹿島切り離し 123→**34**/200(崩壊 18 のうち鹿島由来は 3。元は 20 のうち 10)。**西はほぼ不変**(全域崩壊 8/200 のまま・3 時間 63.3%・サンプル単位では 192/200 が動くが平均は +0.06 pt)。較正が効くのは東だけで、それは鹿島が単一の出口を持つため
- **残る崩壊 18 の原因**は火力の浸水停止(0.3 m 以上で 100%)・17 万 V 未満の変電所・鉄塔の故障で、鹿島を直しても消えない
- **候補デッキ v10**(`nankai_visual_candidates_v10_2026-09-21.*`・29 枚): 「2 m 以下で崩壊する理由」4 枚を追加(引き金は鹿島 / 公表値の 4.5 倍という較正表 / 感度 6 本 / 鹿島の喪失自体は致命傷でない)。v9 の感度表には「この表は run_v8」の但し書きを追加。doctor error 0・GIF 15 本すべて n_frames>1・レンダリング 4 枚を目視確認
- **【続き・負の結果】鹿島の接続を公表どおりに張り直す試み**(`branch_removals` を追加・`branch_additions` の後に実行): 足した= 鹿島火力線 北浜↔鹿島変電所(公表「鹿島変電所—鹿島火力発電所 275 kV 5.0 km」・モデル直線 4.4 km)・香取線 1・2L 鹿島↔新佐原・鹿島線 1・2L 鹿島↔筑波南、外した= 鰐川→新佐原・鰐川→筑波南(OSM 吸着で付け替わった同じ線路)。結果は **全域崩壊 18→14 だが内訳が反転(直後 3→10・後 15→4)**、10 分受電率 87.0→83.3%、3 時間 73.0→77.1%。**津波後の崩壊はほぼ消えるが、揺れの直後の崩壊が増える**。原因は送り出し容量が半減(鰐川経由 7,620 MW → 公表どおり 3,810 MW)で、運転中 4.9 GW が行き場を失うため。**【同日訂正】「真因は 500 kV の欠落」は誤りだった**(TEPCO 設備計画 PDF のパース崩れの 1 行だけで判断した)。独立の 2 出典で否定: OSM 生データ `鹿島海浜線 voltage=275000 circuits=2`、open-keitouzu も東鹿島・鹿島海浜線とも 275 kV。**鹿島周辺に 500 kV は無いのが正しい**([[feedback-verify-before-claiming]])。**本当の真因: 同じ鉄塔に乗る別電圧の回線を全部高い方の電圧で数えている**。OSM の `voltage=275000;66000 circuits=6` を、モデルは 275 kV 6 回線 5,716 MW としている(実際は香取線 2 回線 + 湖南線 66 kV 4 回線)。東京領域の OSM は 7,988 本中 **322 本(4.0%)が電圧を複数併記**、モデル側で名前に「/」を含む枝は 6,467 本中 **603 本**・並列数 4 以上が **388 本 347 GW**。**鹿島だけの問題ではなく全体の容量の過大評価**。鹿島の回廊はモデル 7,620 MW 対 公表の運用容量 約 4,800 MW(1.6 倍)で、張り直し版 3,810 MW は 2 割不足。**既定 run_v9(つながりすぎ)と張り直し版(つながらなすぎ)が真値を挟む**
- **踏んだ罠(環境)**: メモリ逼迫(空き約 285 MB)で `run_dynamic.py` が 7 ワーカー・5 ワーカーとも SIGKILL。西は 3 ワーカー(25 分)で完走。**並行アクターの realtime cron が `git add -A` で私の staged ファイルを巻き込み**、較正と デッキ v10 は `88999f2 data(realtime): ... 2026-09-21 02:30` に入っている(本エントリが実質の記録)。**`.git/rebase-merge/` に 2026-09-02 の autostash だけが残っており `git status` が「You are currently rebasing」と表示する**(実体のない残骸・触っていない)

---

## 2026-09-18 — **Claude Opus 5** — 南海トラフ: 「2 m 以下の浸水で東が全域崩壊する」の真因は鹿島周辺のトポロジーと津波停止規則の仮定(負の結果)

- **オーナーの問い**「ほぼ 2 m 以下で系統崩壊する理由がわからない」「水没のしにくさもあるはず」「もっと丁寧にしたら鹿島が原因にはならないかも」→ レポート `docs/reports/nankai_hazard_2026-09-13/dynamics/kashima_tsunami_artifact_2026-09-18.md`
- **内閣府の一覧表を実読**(令和 7 年 3 月 市町村別ケース別 最大津波高): 東京湾は区部 3 m・川崎/横浜 3 m・横須賀 6 m・神栖 6 m、湾内は「+3 m」が全市区町村で到達せず。モデルの A40 浸水深(湾内ほぼ 2 m 以下・母線 66 局)と矛盾しない。**崩壊の理由は津波の深さでなく、深さから停止を決める規則の側**
- **引き金は東京湾でなく鹿島**: 鹿島火力群(定格 8 GW・運転中 4.9 GW)の出口が 神栖→泉川/鰐川_2→**鰐川** の 1 か所で、泉川・鰐川_2 が A40 1〜2 m。変電所の「1〜2 m で 85% 停止」(**仮定・confidence low**)により 200 サンプルで鰐川 168 回・泉川 171 回停止し、**95 分で鹿島が本体から切れるのは 123/200**(全域崩壊 20 のうち 10)
- **【証拠】業界の公表評価と食い違う**(`hazard/nankai/docs/flood_resistance_evidence_2026-09-18.md`・.yaml、サブエージェント調査・出典 URL と引用つき): 送配電網協議会「南海トラフ巨大地震における電気設備の耐性評価」(2026-01)は 17 万 V 以上を機器の設置高で個別判定し「嵩上げ済みは被害なしと評価」、最過酷ケースの津波被災変電所は **東京電力 PG 0 箇所**・一送 7 社計 5 箇所・被災鉄塔 38 基。電事連 2012「170 kV 以上は津波の影響がある海岸部に設置しない」。耐水高の実数値はほぼ非公表で、得られたのは新仙台火力 3 号「開閉所・遮断器の据付面 GL+3 m」「重要設備は原則 2 階以上」のみ。上限アンカーは 2011 の「構内全面浸水 推定 4 m で機器使用不可」。**1〜2 m 区間の停止率を与える実測は無い**
- **感度(東 200 サンプル・同じ乱数)**: 既定 20 → 代表母線ランク 22 / 停止確率半分 20 / 系統図の直結 30 / **17 万 V 以上は対策済み 16**(3 時間受電率 69.7 → 75.3%、鹿島の切り離し 123 → **1**/200)
- **負の結果 1**: 鰐川だけ救っても崩壊は減らない(20→22)。サイトの浸水深を「敷地内最大」から「代表母線の地点」に変えると鰐川の停止は 168→11 回になるが、泉川(別サイト)と鉄塔の故障で切り離しは残る
- **負の結果 2**: 系統図の直結を足すと崩壊が**増える**(20→30・過負荷遮断 2.54→3.21 本)。公表資料(東京電力 潮流実績・open-keitouzu)では 香取線 1・2L は 鹿島変電所↔新佐原、鹿島線 1・2L は 鹿島変電所↔筑波南/常総/新野田 で、モデルの 神栖→泉川/鰐川_2→鰐川 経由と 鹿島↔神栖 14 回線は OSM 吸着の産物。ただし直結を足しても切り離しは 103/200 残る(発電機の接続先が北浜で、鹿島変電所までが 神之池 0.3〜1 m を通る)。回線数を公表どおり 2 に直しても 31→30 で変わらない。**接続を足すだけでは直らず、この地域は作り直しが要る**
- **結論**: 「東の全域崩壊 10%」は物理でなく仮定の性質(16〜30/200 = 8〜15%)。デッキ v9 の「後で崩れる 17 例のうち 11 例は鹿島の切り離しが引き金」は run_v8 の記述としては正しいが但し書きが要る
- **鹿島の喪失そのものは致命傷でない**: 平常の東(55.25 GW)から 4.9 GW を一瞬で失っても最下点 48.50 Hz・UFLS 1.0 GW で全停せず(緊急融通ありで 48.73 Hz・遮断 0)。2022 福島県沖の再現(6.5 GW 喪失)より軽い。崩壊するのは **UFLS を使い切った状態**のときだけで、1 時間後の UFLS 残りが 3 GW 未満の 7 サンプルは 7/7 が全域崩壊、残り 4.9 GW 未満の 13 のうち 10 が崩壊(崩壊しない 180 の残りは中央値 16 GW)
- **【新規】感度の入口**(既定は無効・`config/dynamics_default.yaml` と `fragility_default.yaml`): `tsunami_site_rank.mode`(max / representative)、`substation.tsunami.protected_kv_min` + `protected_factor`、`fragility_overrides`、`branch_additions.value`。テスト 27 passed
- **次(オーナー判断)**: ①鹿島周辺を系統図で作り直す(鹿島火力の接続を北浜→鹿島変電所・公表 275 kV 5.0 km、香取線/鹿島線の端点、14 回線の是正)②変電所に耐水高 `h_protect_m` を持たせ決定論判定にする(既定値は非公表・感度は新仙台 3 m と 17 万 V 対策済み)③火力の 0.3 m 一律 100% 停止を 2011 実績 9 発電所で較正

---

## 2026-09-14 — **Claude Opus 5** — 南海トラフ: 揺れの到達 → 周波数 → リレー → 系統分離の動的カスケード(run_v2_dyn)・到達表現 3 段階・復旧オペ試作

- **【新規】動的カスケード**(`hazard/nankai/src/nankai/dynamics.py`・`dyn_cascade.py`・`docs/reports/nankai_hazard_2026-09-13/dynamics/README.md`):
  S 波の到達時刻(内閣府 2012 の Vr 2.7 km/s・Vs 3.82 km/s)と津波の到達時刻で設備を順に止め、事象ごとに独立系統(島)を数え直し、
  島ごとの慣性中心周波数 + ガバナ + UFLS + 発電機保護 + 直流の緊急融通 + 過負荷リレーを解く。崩壊した島だけに復電時刻を付けて 90 日の時系列へ。N=200×2 島
- **検算(北海道 2018 ヒンドキャスト・OCCTO 最終報告の事象と整定値)**: 1 回目の最下点 46.74 Hz(観測 46.13)・UFR 1,259 MW(1,300)・道東の単独系統と OFR 停止・北本 593 MW(570)を再現。
  **最終段の全停は再現できず**(残 UFR 約 100 MW の差)。否定された仮定: 火力 47.0 Hz・2 秒 / UFR 時限の等間隔 / 分離 3 秒 / 島分割でタイマーを 0 に戻す実装
- **結果**: 西は 10 分後の受電率 70.7%(感度 71〜82%)、独立系統 平均 1.9・最大 6。10 分で 2 個以上に分かれる割合は既定 57% だが過負荷リレーの扱いで 16〜57% と振れる(分かれる確率は示せない)
- **負の結果**: 東の「ほぼ全域の崩壊」は過負荷リレーの扱いで 4〜55% と振れ、確率として示せない。揺れで東京の火力 12.75 GW が止まった後、66 kV に 5,000 MW(容量 137 MW)の潮流 = 東側の上位電圧経路・降圧点の欠落(既知の過負荷問題)が大きな再配分で表に出た
- **【修正・オーナー判断「1,2 の順で」】東の非物理的な潮流**: 原因は 3 つ。(1) 154 kV 以下のローカル系統を網のまま解き主幹系統の迂回路にしていた → 実運用の放射状(北陸電力送配電 設備形成ルール・九州電力 系統運用ルール)に合わせ常時開路 + 切替送電、潮流は主幹系統だけの縮約(`radial.py`)。最初の「最寄り給電点で区間を切るだけ」は降圧点不足で過負荷 271→357 本に悪化(負の結果)。(2) 北海道 2018 で否定した UFLS 時限の等間隔が MC の既定に残っていた(実装の不整合)。(3) 過負荷判定の潮流で不足を慣性比で配っていた(ガバナ上げ代を無視)。結果: 西は分割 36〜38%・10 分受電 76〜82% と過負荷リレーの扱いに頑健。東の全域崩壊は 6〜34% と振れたまま(鹿島火力が 275 kV のみ・500/275 kV 変圧器 953 MW のデータ欠け疑い)。2022 福島県沖地震の簡易ヒンドキャスト(火力 647.9 万 kW 停止・UFR・全停せず)と整合
- **【バグ】平常状態の偽の分離**: `sim.base_alive`(平常時でも過負荷で線路を切った後)を受電判定に使い、東で 1,209 MW の偽余剰 → 地震前に 110 本を過負荷で切っていた。修正後 平常時過負荷 0 本
- **到達表現 3 段階**(`arrival_physics.py`・`make_arrival_variants.py`): ETOPO1 上の線形長波方程式と √(gh) 走時で到達を二重検算、内閣府 2025 市町村別一覧表ケース②と照合(遠い湾は 1 割・近い沿岸は 5〜19 分遅い)。波源を深くする案は不採用
- **復旧オペレーション試作**(`make_restoration_ops.py`・サブエージェント 1 体、モデル名の記録なし): 班拠点の被災・資機材損失・他社応援の到着。応援規模が過小(東京向け 5.4 班)とオーナー指摘 → 未修正
- **【新規・オーナー判断「1,2 の順で」の 2】復旧の人員を実規模に**(`make_restoration_workforce.py`・`config/restoration_workforce.yaml`・`docs/reports/nankai_hazard_2026-09-13/workforce/README.md`): 配電の電柱折損 × 211 営業所 25,230 人(300 人/百万口・台風15号と能登の実績)× 社内融通 × 他社応援(r < 0.25 の会社だけが送る・災害時連携計画の前提条件)。**建物全壊の巻き込み 0.17155 × 木造全壊率**を令和5年住宅・土地統計調査の市区町村別 建築年次(pws-160core で取得した補助 DB)と 2005 年首都直下手法の曲線で追加 → 折れる電柱 揺れ 4,288 / 建物全壊 62,566 / 津波 83,381 本、配電の停電 1 日後 94→161 万軒(σ 0.3〜0.6 で 141〜214 万)。暗に置く木造全壊 約 108 万戸は内閣府 2025 の揺れ全壊(基本 61 万棟・陸側 128 万棟)のあいだで桁は合う。効くのは応援より社内融通(融通なしで 14 日後 7→47 万軒)。**被害が増えると東京が送り手から外れて応援が 1,089→776 人に減る**(規則の帰結)。未解決: 曲線の σ は仮定(内閣府は図のみ)、営業所の位置は k-means 仮置き
- **【更新】復旧の人員の仮定を補助 DB の実績に置き換え**: 電柱の本数は各社 有価証券報告書 FY2025「配電設備 支持物数」/ 契約口数(東京・関西 0.19〜東北 0.40 本/口、一律 0.25 を廃止)、人員は熊本地震・台風15/21/24号・2022 福島県沖地震の自社+協力会社 / 契約口数の中央値 390 人/百万口(300 から変更)、応援の比率の出典に台風15号 17.8%・熊本 2.0% を追加。結果: 32,799 人・東京が送り手に戻り応援 1,656 人、配電の停電 1 日 / 7 日 / 14 日 = 153 / 90 / 1 万軒。**送り手の判定は仮定の置き方で入れ替わる**(前の版は東京が外れて 776 人)
- **【修正・データの欠け】東の変圧器を東京電力の空容量一覧で置き換え**(`src/nankai/grid_overrides.py`・`grid_overrides.tepco_transformer_capacity`・既定有効): 275 kV 以上 62 本、500/275 kV は照合できた 19 変電所で 18.1→68.7 GW(モデルは 1 バンク分)。台帳の生値は git に載せず補助 DB から読み、帳簿は output/ にだけ出す。275 kV 以上の送電線は比の中央値 1.05 で欠けなし。**鹿島の 275 kV 接続は台帳どおりで、前の版の「欠けの疑い」は外れ**。東の全域崩壊は過負荷リレーの扱いで 6〜34% → 6〜10% に収束、台帳を外すと 32% に戻る。MODEL_INTERVENTIONS.md 登録は別アクター未コミットのため保留(根拠・帳簿・無効化は設定ファイルに記載)
- **【バグ】揺れでも止まる設備の停止を津波の到達まで遅らせていた**(`fragility.py` が浸水のある設備の原因を津波に上書き → `dyn_cascade.events` が津波時刻で停止): 東で 1 サンプル平均 10 基・4.2 GW。揺れの停止フラグを別に持たせ早い方の時刻で停止(乱数の消費順は不変・単体テスト追加)。**run_v4_red 以前は直後の受電率を高めに出していた**: 西 10 分 75.5→66.4%・分割 36→23%、東 94→87%(台帳あり同士)。run_v6 に差し替え(五地域 物理停電 750 / 568 / 388 / 355 万軒)
- **【データの読み】A40 の東京湾は南海トラフ単独の想定ではない**: 神奈川県の東京湾(川崎・横浜・横須賀)の最大クラスは県独自の慶長型地震(国交省 河川分科会 第52回 資料2-5 p.14-16)。東の 1〜2 時間後の受電率低下(88→72%)は湾岸火力の津波停止。湾奥の津波被害を 0 にする下限側の感度で 3 時間後 69→78%(`tsunami_exclude_boxes`)。どちらが正しいかは未判断
- **可視化**: 系統分離の GIF を run_v6 で西・東の 2 本に(描画も MC と同じ初期化関数を使うよう修正。前は台帳の置き換えが入らなかった)。候補デッキ 17 枚
- **【更新】営業所を実在の事業所に**: 補助 DB の utility_offices(各社公表の事業所 456 か所・国土地理院 住所検索で座標化)から本社・本店を除き、母線を同じ会社の最寄りの事業所へ割り当て(k-means 211 か所 → 350 か所)。配電の停電 1 日 / 7 日 / 14 日 = 153 / 91 / 2 万軒、社内融通なしで 14 日後 32 万軒。北陸は支社 4 か所だけで網羅性に差
- **補助 DB の検証と配置**: 人員の較正 5 件・支持物数 2 件・契約口数・変圧器台帳の計 9 件を抜き取り、生データ(HTML・PDF・xlsx・CSV)で値と引用の一致を確認(9/9)。nas03 に生データ `hazard_raw/D1〜D7`・`db/hazard_support.sqlite`・`db/parquet/hazard_support/` と README の行を配置済み(サブエージェント hazard-data-db)。木造全壊率曲線の数値表は県資料でも見つからず(図のみ・負の結果)
- **【修正・データの欠け】中部電力の 500/275 kV 変圧器も台帳で置換(run_v7)**: 中部電力パワーグリッドの空容量一覧(500/275 kV 変電所・単位 MW 明記)で照合 8 変電所(枝 9 本)がモデル 8.6 GW 対 台帳 23.1 GW(比の中央値 0.33)。西の過負荷遮断は 9.0→4.8 回/サンプル、10 分受電 66.4→67.2%・分割 23→21%・ほぼ全域崩壊 3→4% で結論は不変。同じ設定の再計算は乱数ごとに完全一致(run_v7 東 = run_v6 東、中部なし西 = run_v6 西)。五地域 物理停電 720 / 553 / 387 / 354 万軒。関西・中国・四国・九州・北陸・東北は変電所別の台帳が無く対象外
- **【バグ】復旧人員モデルの停電の合計で、復旧対象外(津波で全壊相当)の需要家を二重に数えていた**(`make_restoration_workforce.customers_out`): 送電側の停電確率の分だけ 2 回加算。7 日後 715→523 万軒(送電側 256・配電だけ 48・全壊相当 219)。82ad7e6〜68f1cfc の GIF・デッキ・README の合計は過大だった。感度表の配電の停電は不変。計算をブラウザへ移すために式を読み直して発見
- **【新規・オーナー発意「選択してインタラクティブなツール化」】南海トラフ停電シナリオ卓**(`build_scenario_tool.py`・`hazard/nankai/tool/`・Artifact https://claude.ai/code/artifact/a42d2c8e-6f6d-4a52-8a8d-90996cd7d791): 判断待ちの前提(東京湾の浸水・全壊率曲線の幅)と感度(変圧器台帳・過負荷リレー・人員・応援・社内融通・着手日)を切り替えて比べる。系統は事前計算 12 通り(東 8・西 4、うち新規 5 本 grid_*)、被害と復旧は JS 移植で即時再計算(Python と 4 通りで照合・相対誤差 1e-4)。前提は URL の # で共有できる。東京湾の浸水の切り替えは配電の津波被害と復旧対象外にも効かせた
- **【可視化・オーナー発意「リレー開いたら，明るいの消えていくか色変えることで表現できる？」】灯りで系統の崩れ方を見せる**: シナリオ卓に「揺れが届き、リレーが開き、灯りが消えていく」の再生を追加(12 通りの代表サンプルを再計算・再計算値と MC の記録の一致を検査)。明るさ = 受電の割合、UFLS で削られるほど琥珀色、分かれた島は色、周波数崩壊は赤く光って沈む、孤立は灰、設備損傷は燃えさし。周波数の線・リレー動作の時刻表・受電の内訳を同じ時刻で連動。GIF も同じ描き方(`make_dynamic_cascade_viz.py --style night`、cascade_*_night)で候補デッキに差し替え。足し算の合成だけだと都市部が白く飽和して暗くなるのが見えなかった(にじみと芯の 2 回描きで解決)
- **【表示・オーナー指示「実際に寄せて、母線を選んで完全に消す表示にして」】UFLS を変電所ごとの消灯で表示**(`src/nankai/ufls_display.py`・テスト 4 本): 計算は変えず(島内で一様に削る・周波数と遮断 MW はそのまま)、表示だけ遮断 MW を保って優先順位の高い母線を丸ごと消灯に。優先順位は母線 id のハッシュで固定した仮定(対象の変電所は非公表)。選択は入れ子(遮断が増えても前に消えた母線は消えたまま)。西の代表サンプルの 3 分後は 3,939 母線のうち 932 が UFLS で消灯。シナリオ卓(Artifact Version 3)と夜景版 GIF に適用。トレースのキャッシュは生の受電・島の配列を持つ形(trace_rep_v2.npz)に変え、表示の変換は組み立て時に行う
- **スライドの版名**: オーナー指示「スライドは名前バージョンあげておいて」→ 作り直しは上書きせず `<deck>_v<N>_<日付>` で別名保存。候補デッキは v5(2026-09-15)から
- **【解説・オーナー指摘「1つ1つの専門用語の解析がもう少しあった方がいい」】数式スライド(equations_play.html・Artifact 367cbc76 v9)に用語の解析**: 88 語を板書ごとに「意味・単位と典型値・この解析での役割・注意(混同しやすい語・限界)」の 4 点で書き、板書の下のパネル(1 語ずつ開く / すべて開く)・本文の下線語のポップオーバー(数式の中は除外)・13 枚目の用語集の 3 か所で出す。既存の式と計算は不変。**ついでに直した誤り**: 板書 3 の弱い点「各基の独立を仮定(空間相関で過小評価の方向)」は向きが逆(独立の仮定は「どれか 1 基」の確率を大きく出す安全側)
- **【可視化・オーナー依頼「東京死ぬやつと生きるやつの gif/動画をパワポに」「技術的に何が起こっているか」】東の 3 例**(`cascade_east_{survive,collapse,collapse_early}_night.*`・貼り付け用デッキ `nankai_east_survive_vs_collapse_v1_2026-09-16.pptx` 7 枚・候補デッキ v6): 持ちこたえる #89、津波後に死ぬ #9、直後に死ぬ #59。分かれ目は揺れで失う火力(10.8 / 17.4 / 21.6 GW)と 90.5 分の鹿島の切り離し(4.6〜4.9 GW)時点で残る UFLS の段。GIF は 5 分以降 1 分刻み(`--late-step`)、`_meta.json` に事象の全記録
- **【出力・オーナー「アーティファクトの動画がめっちゃいい。これ出力して。スライドに貼りたい」】シナリオ卓の再生を動画に**(`tool/export_server.py`・`window.exportNight`): 画面と同じキャンバス描画に右の情報欄(時計・受電の内訳・周波数・直近の事象・凡例・前提)を足して 1920×1080 で全 531 フレームを PNG で POST → ffmpeg で mp4(20 fps・26.5 秒)と GIF(1280 px)。既定の前提(run_v8・西 #81・東 #89)を `tool/nankai_scenario_night_default.*` に。候補デッキ v8 に GIF を追加
...
## 2026-09-13 — **Claude Fable 5.1** — 南海トラフ地震 電力ハザードマップ v0(解析ベース A + ポテンシャル法 B)— 新ブランチ `feature/nankai-hazard`・`hazard/nankai/`

- **【新規】正典モデルの上に地震ハザード解析を立ち上げた**(`hazard/nankai/`・レポート `docs/reports/nankai_power_hazard_v0_2026-09-13.md`):
  J-SHIS 南海トラフ最大クラス Mw9.1 の 250m 計測震度 + 国土数値情報 A40 津波浸水想定 → HAZUS 系脆弱性(日本補正)・内閣府 2025 方式の火力停止率曲線 →
  連結成分・需給・DC 潮流・過負荷連鎖 → 対数正規修理時間+作業班+優先順の復旧 → モンテカルロ(N=200)。停電を **物理停電**(設備損傷・系統崩壊・上流孤立)と
  **供給力不足の遮断**(計画停電相当)に分けて出す。自治体集約(N03)・GIF・単独 HTML 地図・内閣府 2013/2025 想定との五地域比較図を自動生成
- **較正(24 変種・`nankai_hazard_2026-09-13/calibration_sweep.md`)**: 直後の桁は zone 供給不足による系統崩壊しきい値が決め(15%→1,100 万軒・35%→550 万軒)、
  変電所曲線は HAZUS×5 で震度階級表(機能停止 ≥1 日)と同水準。採用 = J-SHIS 場・×5・25%。五地域の物理停電 直後 1,001 万 / 1 日 697 万 / 4 日 397 万 / 7 日 358 万軒
  (内閣府 2025 基本 2,060 / 1,350 / 36 / 32 万軒)
- **負の結果**: 4 日後以降は内閣府より 1 桁多く、どの変種でも詰まらない。原因分解で津波浸水域の変電所(約 210 万軒・復旧 60 日)と上流孤立(約 100 万軒)。
  内閣府は津波全壊需要家を復旧対象から除き、配電被害を別勘定にしている。司・翠川式に Mw9.0 直入れは遠方 1 階級過大、HAZUS 曲線そのままは 6強で変電所 7 割停止(熊本・東北実績と不整合)
- **ポテンシャル法(B)**: 最良経路生存率×半径 80km 供給余力。A の 2 日後停電確率と需要加重相関 0.55(単純 0.42)。スクリーニング用途
- **【バグ】DC 潮流の単位換算(×100 の重ね掛け)を「1 手ずつ」トレースで発見・修正(61c32a1)**: 過負荷の誤検出で毎評価 24 本を誤停止させていた。物理停電はほぼ不変(直後 1,001→1,003 万軒)、供給率ベースは 1〜3 pt 低下。単位テスト `test_dc_flow_units_radial` を追加。教訓: 手順を 1 手ずつ見せる可視化は検算装置でもある
- **【新規】1 手ずつビューア**(`trace.py`・`steps_viewer.html`・556 手/サンプル): 損傷サンプリング(p と u の比較)→連結成分→エリア需給 δ→比例遮断→DC潮流と連鎖→作業班割当→時刻再評価を地図上で再生
- 踏んだ罠: pandapower 3.x は `bus.geo`(GeoJSON 文字列)、正典 CSV の RATE_A は全部 100(容量無し)、母線 1 点の座標 NaN、`DataFrame.rank` と列名衝突、A40 gpkg 1.3GB は母線ランクをキャッシュ
- サブエージェント 3 体(モデル名の記録なし): A40/N03 取得と年版選定(市区町村別面積比較)、J-SHIS の download-all.js 展開による URL 規則特定、脆弱性・復旧・較正目標の原典転記(信頼度 3 段)

---

## 2026-09-03 — **Claude Opus 5** — 再生成耐性・介入#38の正典化・F トラック3本（回線数の別名/公表容量の全国化/降圧点の銘板）
...
- **【介入#38 の正典化】潮流だけが正しくて正典のラベルが古い、という捻れを解消**
  （`region_attribution.plan_uniform_freq_flips/apply_uniform_freq_flips`・`apply_node_hygiene.py --freq-fix` 既定ON・
  帳簿 `docs/data/fragments/uniform_freq_ledger.json`・テスト `tests/test_uniform_freq_canon.py`）:
  #38 は**潮流を組むときだけ**効いていて、正典 `all.json` のラベルは古いままだった（**253 ノード**: 群馬 132・山梨 56・
  神奈川 33・埼玉 27・愛知 3・栃木 1・東京 1）。地図・エディタ・輸出は正典を直接読むので、群馬の設備が「中部」と
  着色される実害が残っていた（#42 が混在県に対してやったことの、一意周波数県版）。**島跨ぎエッジ 103→64**（−39＝幻の跨ぎが消えた）。
  **潮流は完全に不変**（pre/post を同一コードで A/B: east 5,375.9MW / 0.8062pu、west 9,252.2MW / 0.667pu が一致）＝
  ラベルだけの是正であることを実測で確認した
- **#42 に潮流ゲートの拒否を追加**（`MIXED_PREF_PF_VETO`）: #38 の正典化で近傍の島構成が変わり、静岡の 2 ノードが
  構造ガード（新規島跨ぎ 0）を通るようになった。しかし適用すると **west ピーク AC の slack/損失が +384MW 悪化**する
  （9,252→9,637MW）。構造ガードは「切れないこと」しか見ないので、潮流で落ちたものは恒久拒否表に理由つきで残す
  （解除するには west ピーク AC を取り直して悪化しないことを示す）。3 状態の A/B（#38前 / #38後 / +2件後）で切り分けた
- **帳簿の上書き設計を是正**（`apply_node_hygiene._merge_ledger`）: 従来は実行のたびに帳簿を上書きしていたので、
  正典へインクリメンタルに適用すると過去のフリップが消えて**逆再生できなくなる**（regen は基底から作り直すので
  上書きでも成立するが、追加適用では履歴が失われる）。id で重複排除しつつ追記マージし、適用回数を `runs` に残す
- **取得の来歴だけを追跡下に置いた**（`.gitignore`）: `data/external/` は再配布不可なので全除外だったが、
  `system_disclosure/provenance.jsonl`（URL・sha256・取得日・バイト数のみ・**生値は無い**）だけを例外にした。
  どのファイルをいつどこから取ったかが残らないと較正が検証できない。生値 4 種で除外の維持を確認済み

- **【トラックF2】介入#44 の未照合 1,725 件の正体を特定し、端点別名表を実装**
  （`data/reference/tepco_endpoint_aliases.json`・`tests/test_tepco_alias.py` 8 件・
  `tepco_endpoint_unresolved_2026-09-03.md`）: 「東京の 66kV 略称を別名で回収できる」という見立ては
  **外れた**。原因別に数えると 匿名コード 1,344（関西 <154kV は公表側が「北CZ」等に伏せている）／
  正典に不在 424／構造マーカー 377（需要家・開放点・〇〇分岐・鉄塔番号）／地域違い 58／
  電圧階級違い 49／別名で解ける 44。東京の未解決は略称ではなく**変電所が正典に無い**のが大半だった。
  別名表には九州の設備番号プレフィックス（「32武雄」＝設備番号32＋武雄変電所）31 件だけを
  `evidence` つきで採録し、匿名コードと不在には**別名を作らなかった**（捏造禁止）。
  ドライラン: 照合 2,079→2,096・更新枝 421→**417**（route が name を上書きしスタブ 2 本が外れる＝
  枝特定的なほうが正しい）・`unresolved endpoints` 1,725→**701**（1,007 件が回収不能と確定）。
  **別名表は回収数を増やさない。増えるのは「なぜ解けないか」の解像度**
- **次に効くのは電圧階級違いの 49 件**: 正典が 1 サイト 1 電圧ノードしか持たないため、
  出典が別階級を指すと `resolve_sub` の階級ガードで落ちる（新豊洲＝出典 275kV・正典 500kV のみ、
  城南＝出典 154kV・正典 275kV のみ 等）。サイト単位で階級を緩める解決規則は
  誤爆ガードとの兼ね合いがあるので、別ゲートを立てて判断すること（本トラックでは変更していない）
- **【トラックF3】線路容量の公表値を全 10 社ぶん取りに行き、介入#45 を再判定 —「係数では揃わない」ことが確定**
  （`scripts/fetch_capacity_tables.py` 新規・`line_capacity_calibration_2026-09-03.md`）:
  未取得だった 6 社（hokkaido / tohoku / chubu / hokuriku / chugoku / okinawa）の予想潮流・空容量表を取得した
  （失敗 0・来歴 182 行を `provenance.jsonl` に `kind:"capacity"` で追記・取得物は All-Rights-Reserved なので
  `data/external` 止まり）。okinawa だけ PDF のみで未判読。ヘッダ駆動の汎用パーサを足して測り直すと
  解析行 **3,968→10,456**・エリア **4→9**・usable な (エリア,階級) **16→37** になり、
  **どの階級も採用規則（3 エリア以上が中央値 ±0.1）を満たさない**ことが確定した（500kV は 8 エリアで 0.365〜0.95）。
  **係数を 2 成分に分解すると理由が出る**: 運用容量÷設備容量（運用余裕）は事業者を跨いでほぼ揃う
  （77kV 幅 0.049・110kV 幅 0.071 で ±0.1 帯を満たす）が、理論容量÷設備容量（導体）は揃わない
  （66kV 幅 0.849・275kV 幅 0.789）。つまり割れる主因は各社の運用方針ではなく
  **電圧階級ごとに単一の `max_i_ka` を振っているモデル側の粗さ**で、熱容量制約の線だけに絞っても
  500kV の幅は 0.475 のまま縮まらない。→ **既定 OFF 維持**。次は係数ではなく
  **公表の設備容量を線ごとに引き当てて `max_i_ka` を置き換え**（#44 の線名マッチャが使える）、
  そのうえで階級ごとの運用余裕（0.50〜0.75）を掛ける形へ作り替える
- **【トラックF4】#43a の推定容量を検証 — 銘板は集まらず、推定規則の過大が判明**
  （`docs/reports/stepdown_nameplate_worklist_2026-09-03.md`・`src/powerflow/stepdown_gap.py`・
  `tests/test_stepdown_nameplate.py` 9 件）:
  71 台が全件推定だった原因は照合の失敗ではなく **銘板索引が全国 13 サイトしかない**こと
  （#43a の 71 サイトは 1 つも含まれず、出典DB に名前があるのも 2 件・どちらも片側欠損）。
  収集を試みたが、東京電力PG 有価証券報告書「主要変電設備」は 500kV 上位 5 所のみ、
  「予想潮流・系統構成」は転載禁止、産業技術史資料は歴史値（新豊洲 1997・東東京 1965）で
  **都心 275/66 バンクの現況銘板は公開系に無い** — 2026-08-10 の負の結果が容量についても成り立つ。
  副産物として **推定規則が出典帯を電圧対ごとに 2〜5 倍上回る**ことを発見
  （東川崎 275/66 は取付線 1 本から 1,000MVA を推定・出典最大は 300MVA）。
  「線の熱容量」と「バンク容量」の取り違えで、#43a の変圧器は事実上制約にならず混雑を過小評価していた。
  出典帯モード（電圧対別中央値・線の熱容量を上限）を実装し**既定は据え置き**。
  同一正典での east ピーク A/B: 挿入容量合計 7,000→**5,900MVA（−16%）**・AC 収束維持・
  slack 5,447.9→5,447.7MW・vm_min 0.8060→**0.8089**・損失ほぼ不変
  ＝**サイズを実態に寄せても解は動かない**（これらの変圧器が現状どこでも効いていないことの裏返し）
---
...
- **【トラックC①】N-1 全枝スクリーニング**（`src/powerflow/contingency.py`・`scripts/sensitivity/n1_screening.py`・`tests/test_contingency.py` 6件）:
  LODF で全枝開放を一括評価（west 6,681 本 2.2s／east 5,919 本 1.9s）。単回線開放を Sherman–Morrison で閉じ
  （`f_j+H[j,k]f_k/(p−H[k,k])`）、橋は分離側の負荷/発電を橋の木で別勘定。**合成定格の錯視を主結果に混ぜない**設計
  （実在線監視の順位が主表・同定タイ/取付線/推定容量変圧器が最悪枝になる開放は別表）。
  実測: 新規過負荷を生む開放 hokkaido **0**／east 183／west 425／okinawa 2（≥154kV 実在監視）。
  上位 20 件を本番 AC で解き直し **全件収束・DC との差 ≤3pt**（east 上野線→上富士線 478→480%、west 山代~久原→山代~日宇 254.7→254.9%）。
  発見: 本四連系（菰池二丁目~福江町三丁目 500kV）がモデル上 1 回線＝橋で、開放すると四国 467 バス・4,031MW が分離
  （実系統 2 回線・OSM circuits 欠落）。east 都心 275kV 環状（上野線）の 1 回線扱いと公表接続直線の標準定格が N-1 で 500% 級を作る。
  → `docs/reports/n1_screening_2026-09-02.md`
- **【トラックC③】運転点込み古典モデルを実装**(`src/dynamics/machine_agg.build_classical_model_ac`): uc_to_pf_built
  本番経路(fy2023r2・west ピーク t=17・#37/#41 込み)の **AC 収束解**から E∠δ = V + j·xd″_sys·I を組み、
  負荷・IBR・静的注入・slack(9,087MW)・停止機の Q(1,446MVar)・容量ゼロ gen の Q・銘板超過機を運転点の
  定アドミタンスに畳んで内部ノードへ Schur 縮約、K=∂Pe/∂δ を運転点で評価。**平衡 Pe(δ0)=Pm が
  機械精度で成立**(最大不整合 3.3e-5 pu@1MVA=33W)。テスト `tests/test_swing_ac_operating_point.py`(9件)が
  平衡・lookup 整合・単位換算・後方互換・解列・2 質点検算をゲート
- **フラット経路の対角項バグを発見・修正**(`build_classical_model`): 旧式 K=−B+diag(ΣB−B_ii) は対角に −B_ii が
  余分に乗り、剛体回転モードを失って周波数が上振れしていた(今日の網で旧式 0.969Hz・inter-area 0 本 →
  修正式 0.443Hz・3 本)。08-17 の公表帯(west 0.375Hz 等)は旧式・別 Ybus(npz v5.0.1)・別機械集合の値で
  直接比較不可。`legacy_diag=True` で旧式を再現できる(テストで固定)。`scripts/pv_dynamics_compare.py`
  もこの関数を使うため再実行すると値が変わる(未再実行)
- **フラット→AC 運転点で何が変わったか(west・運転中 218 機)**: 最低モード 0.537→**0.401Hz**、
  inter-area 帯 2→4 本、帯外 20→3 本(運転点の負荷アドミタンスが網を「重く」して帯が下がり収まる)。
  AC 運転点の固有値は振動不安定 0・負の同期化トルク 0(全 218 機が K_ii>0)。ζ は D_mb=2.0 の**仮定に比例**
  (最小 0.7%・inter-area 中央値 2.8%)— 絶対値は使えない
- **最低 0.401Hz モードは苓北火力(1,400MVA・66kV 接続・K_ii/S=0.19=全機最小)の弱連系局所モード**
  (参加率 1.00 に集中)。苓北を外した残りの同期系の最低モードは **0.469Hz・九州(+0.84)/中国(+0.48) ⇔
  中部(−0.64)/北陸(−0.42)/関西(−0.30)・四国≈0(節)** = 西日本長軸振動の周波数帯と地理形状を
  運転点込みで再現(08-17 の「0.434Hz・九州⇔中部関西」主張を、負荷込み・平衡成立の正しい定式化で更新)
- **N-1 解列の過渡**(`swing_solver.run_transient(fault="disconnect")` 新設=内部ノードの Kron 消去。旧 'trip' は
  Pm→0 で機械が残る別物理・後方互換で残置): 最大単独発電所 川越火力 3,990MW の解列で**苓北のみ失歩**
  (t=1.57s)、残り 216 機は同期維持(最大角差 148.6°)。苓北を定アドミタンス化した A' は安定(147.3°・
  COI Δf −0.88Hz@10s・全地域 −0.87〜−0.90Hz でコヒーレント)。最大集約バス 西名古屋+新名古屋+知多第二
  5,934MW(母線事故相当)は苓北を除いても 199.5° で不安定。**失歩の原因は機械でなく接続先の弱さ**
  (66kV 接続の大型機)= #41(capkv)後も残る接続課題として開示 → `swing_modes_west_ac_2026-09-02.{md,json}`・
  図 `docs/assets/dynamics/west_{trip,modes}_ac_2026-09-02.png`
- **踏んだ罠(3つ・全て平衡ゲートが検出)**: ①停止機(P≈0)を落とすと PV 母線の Q(1,446MVar)が消えて
  平衡が 183pu 崩れる → Q を定アドミタンスで残す ②容量ゼロ gen も Q を出している(藤生町 220kV 28.6MVar)
  ③Q 制限なし PF が小型機に数百 MVar を負わせ(本巣市 50MVA に −564MVar)|E|=0.16〜414・K_ii<0 の非物理
  機になる → 銘板超過(|S|>S_mva・gen 行ごと)は古典機から外して定アドミタンス(50 行・−369MVar・台帳)。
  ④失歩判定は COI 相対だと 1 台の暴走で全機が失歩に見える → 残存機の角度中央値相対に変更
- **右辺のベクトル化**(`swing_solver._Pe/_rhs/_linearise_A`): 二重ループ式と一致をテストで固定。218 機
  10 s の過渡が 0.1 s(従来は Python ループで実用外)。solve_ivp 失敗時は LSODA へ退避・それでも駄目なら明示例外
- **再現**: `PYTHONPATH=. python3 scripts/gen_swing_modes.py --solve-west PKL`(本番経路の solve_hour を
  ラップして解き済み net を退避・ロジック複製なし)→ `--ac-op west --net-pickle PKL --date 2026-09-02`
- **限界**: 古典機(AVR/PSS/ガバナ無し・D 仮定)・型式別典型 H/xd″・合成 R/X・負荷=定アドミタンス(ZIP 未導入)・
...
- **【トラックB④】孤立断片の第三波をドライランで準備**（フォーク trackB4、`fragment_third_wave_2026-09-02.md`）:
  ...
  「927」は全文書に不在＝誤記（正しくは定義(i) 成分 601 / 定義(ii) ノード 1,273）。**適用（親セッション）**: 全 68 本を適用した試行で **east ピーク AC が dc_fallback に退行**したため復元し、`--islands hokkaido west` で **51 本を部分適用**（断片成分 601→550・本系統 +97 ノード・跨ぎ枝 99 不変・hokkaido slack 820.8MW 不変・west AC 維持 slack 9,280→9,306MW）。east 17 本は個別判読まで保留（台帳 #34 追補3）

- **【トラックB①】介入#40（人口メッシュ傾斜）の再判定 — 全国メッシュ整備後も既定OFF維持**（`pop_tilt_rejudge_2026-09-02.md`）:
  `fetch_estat_mesh.py --all-japan` を県ポリゴンの陸域フィルタ（924→181 コード）で 15 分に短縮し、1次メッシュ 150 ファイル・
  人口合計 126,146,090（国勢調査2020 比 **100.0%**）で被覆を解消。標的の江田島市 4 バスは 137.9→**95.2MW（×0.69）**と半減方向に動くが、
  **east/west フルAC（ピーク）は ON で dc_fallback に退行**（08-30 の部分被覆時と同じ症状＝被覆の問題ではなかった）。
  west backbone は AC 維持で slack −200MW。既定は OFF のまま・`scripts/eval_pop_tilt_matrix.py`（`--check-only`/`--assemble`）で再現可。
  仮説: bounded tilt が都市部 66kV へ需要を寄せ、降圧点欠損（#41 east 据え置きの前提と同根）と交互作用して NR が発散する

- **【トラックE1】介入#43 降圧点欠損の是正 — census と 2 介入（#43a 暗黙降圧＝**採用・既定ON**・#43b 低圧網縮約＝実装・既定OFF）**
  （`src/powerflow/stepdown_gap.py`・`scripts/stepdown_gap_census.py`・`tests/test_stepdown_gap.py` 8 件・`stepdown_gap_census_2026-09-02.md`）:
  08-10 の負の結果「66/77kV 層の降圧点は出典で埋まらない」の後、欠損を **(a) 異階級直結**（線 kv の端点に同階級ノードが無く
  別階級母線に直結: east 34 線端/26 サイト、うち 66→275 が 12＝新淀線・西新宿線・西巣鴨線、west 41/35）と
  **(b) 変圧器無しの低圧成分**（east 250 成分/3,210MW・west 496/5,117MW＝合成無限大母線が給電）に分けて数えた。
  **#43a** は (a) に同サイトの kv_L 母線+変圧器を挿入（存在は電気的必然・容量は銘板 0/71 で全件 @推定）: east 静的 AC
  vm_min 0.819→**0.857**・実在線過負荷 353→342・UC 断面 slack −22MW、west 中立。**#43b** は (b) の負荷を最近傍 ≥110kV へ
  帳簿付きで縮約: R=5km で east 2,142MW・west 3,267MW が実網に載り、輸送損失がそのまま slack/損失に出る
  （east +232・west +580MW）。移設先は経路の推定なので既定 OFF（フラグ `--lv-aggregate R`）。
  電圧不明ノード（仮置き 66kV）へ 187kV 幹線が繋がる junction は降圧点でなく、接続線の階級へ置き直す（`reclass_unknown_kv_buses`）
- **#43a を既定 ON にした（親セッションの判断・2026-09-02）**: 66kV 線が 275kV 母線に直結することは電気的にあり得ず、降圧変圧器の**存在は必然**。これは #37（(仮)都心給電）でオーナーが承認した論法「仮が事実でないかもしれないなら、それを明記しておけば正典として良い」より**弱い主張**（#37 は経路まで仮定するが #43a は同一サイト内の設備の存在のみ）なので、同じ基準で既定 ON とした（`IMPLICIT_STEPDOWN_DEFAULT`・無効化 `--no-implicit-stepdown` / `AJG_IMPLICIT_STEPDOWN=0`）。容量は 71 サイト全件が推定で、変圧器名と帳簿に @推定 を明記する。あわせて **#45 の較正 ON 時に暗黙降圧の推定容量が縮む問題**を`_line_mva` が `max_i_ka_theo`（較正前の理論定格）を読む形で解消した（E3 の指摘）
- **#41 east 据え置きの再判定**: #43a を入れても east の capkv は cap より実在線過負荷が +86〜+114 本多い（OFF 353 vs 439・#43a 342 vs 447・
  #43a+#43b 354 vs 468）→ **east=cap 維持**。降圧点は #43a で都心 66kV 群の一部しか埋まらず、「接続電圧を正すほど降圧点へ集中」の
  交互作用が残る。#43b の移設先を出典で置換した後に再判定
- **副作用の是正**: `add_per_component_slacks` が非通電バスも成分に数えて 1 バス 1 合成スラックを立てていた
  （#43b で初めて非通電バスが生じ発覚。解には影響なし・件数だけ膨らむ）→ `in_service` バスのみに限定
- **【トラックE2】介入#44 回線数（par）の出典補完を適用**（フォーク trackE2、`circuit_sources_2026-09-02.md`）:
  OSM の `circuits` タグが無い線は 1 回線扱いで、C① の N-1 で本四連系（実系統 2 回線）が橋になっていた。
  一般送配電事業者の公表資料（関西 空容量 CSV・東電 予想潮流 CSV 基幹+13 エリア・四国 系統容量 CSV・九州 31 地区
  空容量一覧・各社インピーダンス表の回線別行・open-keitouzu）から **4,913 レコード**を読み、端点変電所間の同階級最短経路（route）
  と線名一致（name）で正典の枝へ引き当てた（増やす方向のみ・quote/URL/取得日を `data/reference/circuit_counts.jsonl` に）。
  **421 枝を更新**（1→2 が 383・500kV 22 枝・275kV 24 枝）。本四連系 1→2・上野線/上野水道橋線 1→3。
  **N-1 が実態に寄った**: west の基準過負荷 62→35・新規過負荷を生む開放 425→187・分離負荷 37,109→31,777MW（本四連系が橋でなくなる）、
  east の新規側最大負荷率 503.6→232.2%（上野線の 500% 級が消滅）。ピーク AC は両島維持（east slack +88MW・west −48MW）。
  容量値はコミットしない（All-Rights-Reserved 方針）。山代~久原「500kV」は公表が 66kV 2 回線＝OSM の電圧タグ疑い（据え置き・要判読）
- **未照合の正直な内訳**: 2,834 レコード＝線名がモデルに無い 926（関西 500kV 幹線の公式名は OSM に無いが、インピーダンス表の端点で route 照合済み）・
  端点未解決 1,725（東電エリア CSV の 66kV 端点略称・四国/関西の匿名コード「変2」「BD」・OSM 欠落の 岐阜開閉所/東栄/智頭）・
  kv/地域不整合 84・近接拒否 99。次に効くのは端点略称の別名表（東電 66kV）と OSM 欠落変電所の補完
- **【トラックE3】線路容量の運用容量較正を全国化・介入#45 として実装（既定OFF）**: 08-09 の関西のみの較正を
  kansai(≥154/<154)・shikoku(基幹/ローカル)・kyushu(31地区)・tokyo(基幹+13地域) の 3,968 行に広げ、**比だけ**を
  `config/line_capacity_calibration.yaml` に置いた（生値は data/external に留める）。154kV は関西/東京で **0.679/0.678** と一致、
  275kV 0.45〜0.54、66kV 0.58〜0.76、**500kV は 0.37〜0.95 と割れ**（東京=大容量導体、四国=安定度限界）、
  **110kV は 0.27**（`max_i_ka` 0.9kA が実態の 3〜4 倍）。判定規則（3 エリア以上が同階級で ±0.1）をどの階級も満たさず
  6/10 社が未取得のため**既定 OFF**（`--cap-calib`／`AGJ_CAP_CALIB=1`）。**設計上の発見**: 係数を `max_i_ka` に乗じるだけだと
...
- **北海道の真因究明と是正候補**（オーナー指示「これは重要な内容なので北海道調査と改善」）:
  真因は **`cap` が電圧階級を見ないこと**だった。判定は「バスに集まる枝の**合計**容量
  >= 出力」だけなので、枝が多ければ 66kV でも選ばれる。**京極発電所400MW（実系統は
  275kV・西双葉開閉所）が札幌市南区の66kVバスに載り**、68.6MVA定格・255mの同一敷地タイに
  218MW を流して 318% を作っていた。当初見立てた「同定タイの定格が artifact」は
  **症状であって原因ではなかった**（66kVに218MWを流す構成自体が誤り）。
  端的な指標=200MW超11台6,989MWのうち **cap は2台650MWが66kV以下に載る**（capkv/kvfitは0台）
- **是正候補 `capkv`（cap ∧ kvfit）を実装・既定は据え置き**: hokkaido 318.0→**86.3%**
  （過負荷0本）・west 1102.7→**693.9%**・okinawa 不変。発電は全モードで保存
  （hokkaido 417台10,574.6MW同一、66kV接続が2,863→1,861MWに減り187kV以上が7,331→8,333MWへ）。
  **ただし east は 725.5→1031.4% と悪化**し、最悪線は「新淀線 淀橋(66kV)→新宿(275kV)」＝
  **66kV↔275kV をまたぐ降圧点そのもの**。既往知見「真因は接続電圧**と降圧点**の欠損・
  交互作用で符号は反転する」が再現したため**一律の既定変更は不可**とし、島別既定の可否は
  オーナー判断に残した → `docs/reports/hokkaido_cap_attach_regression_2026-09-01.md`
- **踏んだ罠**: `balance_by_zone` 後の `net.gen.p_mw` は**運転出力**であって設備容量ではない
  （京極 400→176MW）。比較段階を取り違えて「capkvで容量が変わった」と誤読しかけ、
  独立に数え直して訂正した（驚く数値は独立検証してから報告、の実例）
...
- **試験#1×#3**(オーナー「試しでいい、1と3見たい」): アンテナ集約(<100kV deg-1連鎖2,103バス26%・需要保存)で**west素朴ACがフラットスタート0.2秒収束**・vm_min0.725→0.927。低電圧テールは全てアンテナ側と実測(幹線min0.862)。様式5変圧器213基は単純照合7基のみ=照合が律速。概念図・実地図・電圧分布・スライド7枚を納品
- **追試「アンテナのまま計算できないか」**: DC初期化/fdbx/warm start全滅→6段フォールバック切り分けで**west全規模「AC収束」の実態=tol10MVA緩解**と確定(第6段のみ成立・0.04s・vm一致で同一解同定)。warm start失敗=緩解が厳密解の吸引域外(近傍に真解が無い示唆)。fdbx即死=下位網R/X比でFD仮定破れ。本丸=LTC/調相のモデル化 or 末端R/X精緻化
- **全試行の完全記録**(オーナー指示「失敗も全て記録」): 23試行の表(アルゴリズム・設定・時間・原因解釈)+MATLAB検証の失敗記録(batch終了時abort()・壊れセッションのloadcase誤診)を報告書付録に。ハーネス6本=scripts/diagnostics/・結果JSON=docs/reports/

---

## 2026-08-19b — **Claude Fable 5** — 手法(a)実装: エリア需給実績の燃料別注入 — 原発停止等の時事を公表実績で自動反映

オーナー「時事ネタは反映させる必要ありますか?」→方針=一次データ自動反映→「ちょっと整備して」。

- **取得器** `fetch_area_fuelmix.py`: 9/10社のエリア需給実績(燃料別30分値)のURLを実fetchで特定(東京中部北陸中国四国九州=月次CSV日次更新/北海道=日別当日/関西=当日JSON万kW→蓄積/東北=2か月遅れ/沖縄=未特定)。+OCCTO kohyo_04計画潮流(FC・北本・当日あり)。方言吸収=NFKC・北海道語彙・関西系列名
- **注入** `export_day_flows.py`(a4・既定ON): 発電=zone×燃料実績を容量比例/太陽光不足分は需要側控除(分散太陽光の物理近似)/充電は需要加算/島間はkohyo_04を変換所バス注入(BOUNDARY_POINTS共有)/フォールバック明示。関西の需要TODOも副産物で解消(エリア需要列で時間単位補完)
- **検証**: ゾーン純位置vs公表連系線列=中国39MW・四国129MW・中部~70MW(FC除き)。8/18断面で九州原子力2,948MW=川内玄海・中国0=島根停止が手入力なしで再現
- **踏んだ穴3つ**(実測検出→修正): 部分需要zoneの片肺注入/境界帰属ノードへの全量適用(帳簿h=96)/太陽光clipのゾーン歪み(東近江5,106MW幻潮流)
- 限界: 単線値のループフロー(東近江vs三岐の配分はインピーダンス依存)・NOW断面は未適用・レポート=fuel_actuals_injection_2026-08-19.md

---

## 2026-08-19 — **Claude Fable 5** — 大型連系線・変換所の重点監査→3件是正(UC収支等式化・yaml正本化#33・南福光BTB切断#32)

オーナー指示「大きな連系線や変換所は特に重点的に確認」→監査で3問題を発見、続く「3件は直しておいて」で全て是正。

- **監査**(`interconnector_converter_audit_2026-08-19.md`): 変換所在庫は函館変換所欠落(開示済)以外健全・50/60Hz分離健全。UC代表日断面で関門5,791MW(運用容量の2倍)・東岡山西播8,437MW/ld128%・本四2,401MWの超過を発見。実績需要断面は概ね容量内=UC断面固有
- **真因**: UC地域収支が`>=`(不等式)=**余剰の自由廃棄**。九州昼は発電8.2GW−純需要2.5GW=純輸出5.7GWなのにスケジュール関門2.78GWという帳尻の合わない解が「Optimal」だった。加えてuc_snapshotが揚水充電3.5GWを落とし、島内DC(阿南紀北700MW)は非注入で本四へ上乗せ
- **是正①**: 等式+spill変数(ペナルティ・regional_spill_mw帳簿)/充電を需要側計上/島内DC±ペア注入(INTRA_DC_POINTS)。検証=全地域収支残差0.0・全10連系×24hをcap内・west h12で関門-2,999(≈スケジュール)・東岡山西播4,752<4,850・本四1,145<1,450
- **是正②(介入#33)**: interconnections.yamlをOCCTO正本から再定義。ic_005中部北陸=南福光BTB300MW(旧「加賀〜越前1,900」は北陸域内線+越前嶺南線容量との混同)・ic_010北陸関西を追加・関西四国=HVDC化・**方向別容量**導入(関門 順850/逆2,850)。三重東近江のOCCTO順方向=関西→中部(検証済み対応表)。fy2023r2/fy2025r1に先行ワークアラウンド(override300+addition ic_010)が存在=独立同定。fy2023の凍結解除・旧yamlはlegacy保存
- **是正③(介入#32)**: build_island_netのBTB_SPLITSで南福光連系所バスを分割(越美幹線→中部側・既定ON・--no-btb-split)。A/B=素通し575MW→0・AC収束/vm_min不変。罠=分割バスをtype="b"で作ると県別需要1,556MWが集中(type="n"が正)
- テスト: UC系474件パス(件数固定2件は10本へ更新・fy2023凍結テストは正本継承に書換)
- 残: flow_map出荷データ(p24)のallhours再生成
...
- **①東京23区の線名逆引き両端法**: 潮流実績(tokyo_23_01/02)で「A局名を含む線名」を逆引き→
  同一線名が2局で計測されればその2局が両端でA局は中間。9局13接続(千石町—墨東/江東・茅場町—永代橋・
  福住—永代橋/江東・南池袋—豊島・東海町—南大田/天王洲・上富士—田端/豊島・大井—天王洲・
  板橋—常盤台/戸田・美野里—石岡)。**罠14回避で不採用2**: 巣鴨(西巣鴨変電所が別局で実在)・
  前野(北前野の素性不明)・天野(群馬の同名線=同名別所)・王子(八王子の部分一致)
- **②中部77kV長野図の判読**: 松本圏の無名孤立7ノードが**全て実名chubu局の30〜225m隣**=
  tokyo/chubu二重登録(島田と同型・F類は名前不一致で見落とし)。同一敷地タイ+fix_region: chubu
  (文字列指定に拡張)で7局同定。出川二丁目は図の「出川」=中信52150幹の分岐→中信へ接続。
  **県一丁目は77kV/44kV長野図とも不在**→公表不在の証拠でA降格提案(overrides27件目)
- 副修正: ①route台帳を累積マージ化(置換N件の正常実行が過去12件記録を消しapply防波堤が
  穴になる事故→gitから復元) ②スタブ0本置換はスタブ実在で判定不能→「両端点が正典頂点として
  実在」も置換生存の証拠に追加 ③汎用名(tokyo_sub_NNNN)はfrm_nearヒント時のみ純座標≤300mで拾う

## 2026-08-16(2) — **Claude Fable 5** — 大工事: OSM再抽出→DB統一ループ実運用→幽霊治癒で main+393

オーナー承認「大工事していい」「サーバー/nas03使っていい」「OSM以外の情報からの補完もあり」「マップから探すもいいね」。
- **基底刷新**: 全10地域lines+subsをosm_id+全タグ付きで再取得(fetch_osm_refresh.py)。移行は
  幾何一致の名前引き継ぎ(migrate_osm_refresh.py)で未解決ノード0。変電所+118九州(小倉圏)等
- **DB統一ループ初運用**: enrich封鎖(正=grid.db)と判明 → db ingest(キュレーション243,093行復元)→
  db enrich --lines(**消失名1,398本を全数再命名**)→db export → drift試験13 passed
- **罠15基底刷新版**: way編集の重心移動で帳簿座標34箇所が幽霊化(「適用済みなのに孤立」)。
  恒久修正=適用時に最寄り現ノード≤500mへスナップ。幽霊エッジ33本を外科除去→治癒
- **罠17**: route防波堤が初期状態で接続12本を消す入れ子欠陥→「スタブ実在時のみスキップ」に修正
- **プールスイープ**: 新基底で両端実在・未接続267件→片側孤立5件を全接続(千代/穴生/通谷/富士見/荘園)。
  F類が刷新後の新同一敷地ペアを自動検出(+25タイ・region是正+56)
- **最終KPI**: 本系統外1,688→**1,801**(実在発見+524ノード込み)・main15,668→**16,061**(+393)・
  孤立457→499・A75→123(刷新直後175からの治癒)。帳簿192本・PF全4島AC収束維持
- 次期構想を記録: 変電所の電圧階級(1次/2次)入り口出口+**変圧器バンク的モデル化**(オーナー提案)

## 2026-08-16 — **Claude Fable 5** — 九州31地区プール全面化: 残Aの真相は「公表の欠落」でなく「住所合成名のartifact」だった

引き継ぎ項目⑤の実施(`reports/kyushu_pool_campaign_2026-08-16.md`)。

- **プール構築**: `scripts/pool_kyushu_kuyoryo.py`(新規)で31地区の予想潮流・空容量一覧表から
  送電線831行(from→to実名)を機械抽出。接続事実=`kyushu_kuyoryo_pool_2026-08-16.json`
- **解決3本+同定1件**: 犬飼(西大分—大南—犬飼・大南は供給ノード補完17局目)・
  湯山=モデル「天瀬町赤岩」(OSM無名wayの住所合成名を、隣接する湯山発電所way+日田図/表で九電名に同定)
- **仮説棄却→真相**: 「敷川内/岱明/天瀬赤岩はプールで解決できる」は棄却(公表62文書に名前が無い)。
  真相はモデル側: 岱明町野口=**き電変電所(traction 20kV)**をkv=66と誤合成・敷川内町=ソーラー連系疑い。
  overrides提案2件追加(計26件・承認待ち)
- **第4の解読鍵**: 西日本のOSM無名変電所は住所合成名で事業者名と一致しない。
  「名前が資料に無い」で打ち切らず座標近傍のOSM実体で同定する。逆に公表一覧への不在自体が
  「66kV系統ノードでない」証拠になる(配電用変電所は66kV受電なら全記載のため)
- **スクリーニング資産**: プール端点530×モデル突合→真欠落候補171(沈堕・東豊前・相浦等)=
  `kyushu_kuyoryo_gap_2026-08-16.json`。自動採用せず(梃子人間判断)
- KPI: 孤立460→**458**・A78→**76**・本系統外1692→**1689**。九州A残6は全て提案済み=公表で繋げる分は完遂
- **第2部(日田コンプリート+自己訂正)**: 日田表16線を全数モデル化(補完+4局=夜明/野上/大岳/下筌・
  接続+4本・帳簿161本)。前回判定「日田市変電所110=下筌」を**自己訂正**(=大山。OSM接続110kVのみ+
  地区唯一の110/6kV局+図位置の独立3証拠)・_2=三芳と同定。**新罠16**: kv優先pickの非決定性×累積帳簿で
  同一YAMLエントリが別エッジを複製(端点ペアdedupをすり抜け)。発覚は「新規3件・足したのは2件」の勘定ズレ
- **第3部(OSM実線形吸着・オーナー指摘)**: 「人間には細い線が見える。線があるものは地形的に辿ってほしい。
  これは全部に言えること」→ `route_disclosure_edges.py` 新設・STEPS組込。実証コード90本を全数診断:
  **12本は宙に浮いたOSM断片=公表線そのもの**(off-main比率1.0・四国東部チェーン8本+泉北/三田/牛久)→
  取付スタブに置換し実線形でmain合流。別線迂回33本+未収載45本は直線維持(捏造しない)。
  KPI不変(同じ接続の実経路表現)・PF/Ybusは実線長ベースに改善。台帳=routed_disclosure_edges.json。
  副次修正: 置換0の再実行が台帳を空上書きする穴を防波堤(帳簿空上書き事故と同族)
- **第4部(中部実施・オーナー指示)**: **2つの誤認を訂正**: ①「中部は仮名化」→西部/北部変電所等は
  500kV局の**実名**(総称っぽい実名を仮名と誤認) ②「潮流実績未発見」→rule_NN連番Excelに2018年度〜あり、
  **ローカル77kV以下(557列)まで実名**の潮流正方向+1時間値=東北・東京級の最上位開示。13ファイル取得・
  `convert_chubu_flow.py`(Excel→標準CSV・恒久)・registry登録 → 観測1,516行・**viz配置0→381本**(全国2,554本)。
  島田電力所(review帯425m・tokyo/chubu二重登録)を中部実名開示の独立証拠で同定し
  **fix_region機構を新設**(島跨ぎタイ無効化の回避)→孤立457/A75/本系統外1688。
  住所合成名⇔事業者名の**エイリアス台帳 config/utility_name_aliases.yaml 新設**(沈堕=大野町小倉木・
  東豊前=豊前市・相浦=光町・古月=大字木月等7件) — 「真欠落171」の相当数は命名対応の問題と判明。
  防波堤追加: route置換後のad-hoc applyがコードを再追加して並列偽回線を作る穴を封鎖

## 2026-08-15 — **Claude Fable 5** — 孤立変電所の一部は検証器の人工物だった: 島判定バグ修正(+222復帰)と実証接続v2(worklist82本)

深夜の自律作業(オーナー指示「数日の作業および再開および改修を7時までに実行」)。

**バグ2件を検算で検出・修正**:
1. `reconcile_isolated_multi._latest_per_family` の族キーがファイル名のみで社をまたいで
   衝突(東北 jisseki_kikan01 が北陸の同名2025年版に食われる)。東北の解決4件が
   全地域再実行で0件に退行したことから検出。修正で東北プール391→955本。
2. `compute_connectivity` の島判定が座標キー先勝ち(`island_of.setdefault`)で、同一座標に
   別島ラベルの重複コピーがあると片島の枝が黙って捨てられていた(東西境界203キー+
   北海道/東北17キーを実測)。**下北半島全域がこのバグで孤立扱い**(OSM線は存在した)。
   島ごとのキー集合に修正 → **本系統外2107→1885(222ノード回復)**。
   `connectivity_island_key_collision_2026-08-15.md`

**実証接続v2(介入#29・worklist・適用待ち)**: 東北NW潮流実績3年分(kikan+local01-07・
line/tr)取得→公表線10本(下北半島チェーン)+分岐タップ2本(飯舘→原町・東通)+
変圧器台帳201基+**同一敷地同定70本**(同名・同電圧・≤300mの跨region二重登録=
中部↔東京境界)。region是正89ノード(青森箱=下北半島のhokkaidoラベル20を含む)。
ドライラン **1885→1799(86減・孤立変電所75合流)**。同名誤マッチ(茨城の大畑変電所)を
region優先pickで回避した実例つき。`disclosure_connection_worklist_v2.json`

**撤回1件**: 「275kV標準X/R過小(17.50 vs 11.61)」は**n=6照合サブセットの偏り**。
公表全数54本の中央値12.78は標準11.61と整合し修正不要。187kV(#27)は全数107本・
両社独立一致で採用推薦のまま。`impedance_calibration_review_2026-08-15.md`
...
## 2026-08-15(第3部) — **Claude Fable 5** — 全エリア判読キャンペーン: 判読エージェント6体で孤立A139→94

オーナー指示「他のエリアも同様な作業を全てやればいい」。各社公表の系統図・空容量マップ
約70文書を取得し、判読エージェント6体(捏造防止契約つき)を並列展開。関西の匿名コード
解読(A表×B表)・東京CSV両端法(v1と同一手法で校正)・九州のリンク注釈発掘(連系マップに
全31地区図URL)等で**45本の実証接続を追加適用**(帳簿累積121本)。

**孤立変電所 533→487・繋ぐべきA 139→97**(セッション累計 705→487 / 257→97。初報484/94は同名別所誤pickの偽エッジ2本による水増しで、距離監査により訂正)。
PF全島AC維持・east vm_min 0.850→0.873・成分 east302→276 / west541→492。

構造的発見: ①**西日本の孤立主因は隣接ノード自体の欠落**(四国66kV網の約2/3が未収載・
判読確定した相手端が九州8局中1局しかモデルに無い)→次はノード追加キャンペーン
②郷ノ浦(壱岐)=離島独立系統で孤立が正 ③region誤ラベル4件確定(中央南=田布施等)
④JR東自営線説を否定=東上越/高田/小出は東北電力NW設備 ⑤TEPCO 23区66kV図は10系統
のみ収録で都心は非公開 ⑥kvタグ誤り多数(66↔77・22kV系の66表記)。
`mapread_campaign_2026-08-15.md` / 一次出力=`docs/reports/mapread/`。

同日の事故2件を検出・修正: 帳簿空上書き(→--update-ledger明示+累積マージ)・
YAML末尾改行欠落によるエントリ乗っ取り(1,000km偽エッジ→3点修復・罠13)。

## 2026-08-11 — **Claude Opus 5** — 未調査だった電圧悪化を追跡: 犯人は#25単独、係数0.6が陳腐化していた

08-10 に「hokkaido と沖縄の電圧品質が悪化（vm<0.80 が 8→38バス）」を記録しながら
**未調査**のまま残していた。追跡した（`reactive_factor_recalibration_2026-08-11.md`）。

**犯人は介入#25 単独**。hokkaido で介入を1つずつ積むと:
6月相当 8本 → +#24 **8本（無影響）** → **+#25 45本** → +#26 45本（無影響） → +出典付き容量 38本。

機構: モデルの発電機は `max_q_mvar = 0.5×容量` を持つ**無効電力源**でもある。
水増し太陽光を消したことで**捏造されていた無効電力サポートが一緒に消えた**。
介入#20 の係数 **0.6 は太陽光が180GWあった頃に決めた値**で、いまのモデルには合っていない。

**係数を 0.6→0.8 にすると4島すべてで改善し、悪化する指標がゼロ**:
  hokkaido vm<0.80 **38→1**・帯 74.4→85.2%・最大負荷率 96.8→93.3
  okinawa       2→**0**・74.5→79.6%・193.6→185.9
  east          0→0・99.0→99.3%・757.5→755.7
  west         11→10・99.2→99.5%・740.1→739.6
hokkaido は 0.9 で 0本/88.0%。**0.8 で6月水準（8本/85.9%）とほぼ同等（1本/85.2%）に戻る**。

**ただしこれは「捏造の付け替え」**: 係数を上げる＝コンデンサバンク（介入#20・★★）を増やす
（4島計 +9,000MVar弱）。ただし**後者は数えられ・開示され・切れる**（`--no-reactive-comp`）ので、
「実在しない太陽光170GWのq（未開示）」より台帳の原則に沿う。**未適用・採否は人間判断**。

**測定器の誤りを1件（自分）**: 最初 `solve_island` の**第1返り値（DC net）**の vm を見て
「全構成で vm=1.000・悪化なし」と出した。DC に電圧は無いので当然で、第3返り値が AC。
**west が黙って落ちた**ときは同僚セッションの作法どおり単独で再実行して解決した。

## 2026-08-10(続7) — **Claude Opus 5** — 訂正: 「west が AC 収束」は誇張だった

`01df4af` のコミットメッセージと報告で「**west が AC 収束した**」を成果として書いたが、
裏を取っていなかった。`diagnose_pf_frontier` を回すと:

| 島 | 成分 | 合成slack | 最大成分の**負荷占有** |
|---|---:|---:|---:|
| hokkaido | 35 | 32 | 93.7% |
| east | 312 | 290 | 91.6% |
| **west** | **541** | **490** | **89.6%** |
...
west の「収束」は **541 個の断片がそれぞれ自前の合成 slack（実在しない電源）で
局所的に解けた**形式的収束で、`docs/WEST_AC_ANALYSIS.md` が「fragmentation による
見せかけ」と確定させたものと同じ。**単一の網が解けたのではない。**

ただし east（91.6%）も同じ形式であり、west だけが特別に悪いわけでもない
— `pf_frontier` の主旨どおり「収束したか」は負荷占有と併せてしか読めない。
正しい言い方は「**west も他island と同じ形式で AC が解けるようになった**（負荷の 89.6% が
最大成分）」であって、「west が AC 収束した」と単独で書くと実力を誇張する。

**あわせて記録すべき電圧品質の変化**（今日の介入群の影響・`pf_frontier_2026-08-10.md`）:

| 島 | vm 0.95-1.05 | vm<0.80 バス |
|---|---|---|
| hokkaido | 85.9% → **74.4%** | 8 → **38** |
| east | 98.5% → **99.0%** | 21 → **0** |
| west | 99.1% → 99.2% | 13 → 11 |
| okinawa | 100% → **74.5%** | 0 → **2** |

east は明確に改善、**hokkaido と沖縄は悪化**。太陽光の水増しが隠していた弱点の露出という
既報の機構と整合する（`354607d`）。小島ほど分散電源の除去が効く。

## 2026-08-10(続5) — **Claude Opus 5** — 出典の伝播を修復: 合成率 20.1%→**14.0%**、実容量ゼロの地域が消えた

オーナー方針「できることを埋めていく」。**埋めたのは「無いデータ」ではなく「繋がっていない配管」**。

**穴**: 1-C GEM 充填で作った出典付き容量 **350 件（227GW）**は D層
`docs/data/plants_all.geojson` に入っていたが、潮流も CIM も R層
`data/*_plants.geojson` を読むため**誰も使っていなかった**。
2026-08-09 の監査が「`capacity_mw_sourced` を持つのは 0 件」と指摘していた穴。
その結果 **九州 2,549 件・沖縄 32 件が全件 `capacity_mw = -1`**＝実容量ゼロ、
九州の 23,147MW は丸ごと既定値だった。

**塞ぎ方**: R層は書き換えず（層の分離を守る）**読む側がD層を引く**。
座標キーは `apply_capacity_sources.py` と同じ規約で、実測 **350/350 一致・重複 0**。
出典値 0（大間原発＝運転開始未定）は **0 のまま尊重**し既定値へ落とさない。
既定 ON・無効化 `--no-sourced-capacity`。潮流に届いた量は west 247件/110,021MW・
east 62件/87,060MW・hokkaido 11件/4,989MW・okinawa 10件/2,274MW。

**素性の改善（今日の推移）**:

| | 開始時 | #25 既定ON後 | **出典伝播修復後** |
|---|---|---|---|
| 合成率 | 48.3% | 20.1% | **14.0%** |
| 総容量 | 477,173MW | 308,655MW | 339,182MW |
| kyushu 合成率 | 100% | 100% | **33.3%** |
| okinawa 合成率 | 100% | 100% | **3.9%** |

**「実容量ゼロのエリア」が消えた。**

**潮流への影響は小さく混在**（east 747.8→757.5% / okinawa 203.0→**193.6%** /
hokkaido vm下限 0.631→**0.656**）。`balance_by_zone` が需要へ正規化するので当然で、
価値は潮流値ではなく**素性**にある。

**監査も同じD層を引くよう直した** — 片方だけ直すと「潮流は出典値を使うのに監査は
九州の実容量ゼロと報告する」食い違いが残る（実際に一度その状態を作った）。

**テストの向きを2本入れ替えた**:
- `test_synthetic_share_is_disclosed_not_hidden` は「kyushu と okinawa は 100% 合成」を
  固定していた。解消したので **「実容量ゼロのエリアは無い」**を新しい不変条件にした
  （1 エリアでも 100% に戻ったら伝播が切れた合図）
- 特性テストが**三度目の発火**（128.8→136.3%）。原因を出典伝播と特定し理由を書いて更新。
  cap 側は 87.1→88.4% とほぼ動かず、接続規則が効いている裏づけにもなった

gate: 1,276 passed / 0 failed / 3 skipped

## 2026-08-10(続4) — **Claude Opus 5** — 介入#26→#25 を既定ON化: 合成率 48.3%→20.1%、ただし清々しい勝ちではない

オーナー承認（「実装素晴らしい。進めて」）で **#26 → #25 の順**に既定ON化した。

**介入#26（発電機の計上エリアを operator で決める）を既定ON**
定数 `GEN_ZONE_BY_OPERATOR=True`・無効化 `--no-gen-zone-by-operator`。
`balance_by_zone` の引数既定は False のまま（what-if 群のベースラインを守るため・#24 と同じ形）。
感度3本と `diagnose_pf_frontier` も同じ定数を渡すよう配線した。
これで **#25 の唯一の実害だった kansai の供給不足が消える**（容量/需要比 0.98→**1.47**、
hokuriku の異常な 5.00→2.24、全ゾーンが 1.5〜3.9 に収まる）。

**介入#25（太陽光の既定容量 10→0.10MW）を既定ON**
写し4箇所（潮流本体・監査・snapped_topology・テスト）を同時に是正。
**合成率 48.3%→20.1%・総容量 477,173→308,655MW**（④の静的棚卸しの予測と完全一致）。

**本番潮流の実測 before/after（AC）** — 混在している。隠さず出す:

| 島 | 旧（#24のみ） | 新（#24+#25+#26） |
|---|---|---|
| east | 853.4% / vm下限 0.757 | **747.8%（−12%）/ vm下限 0.824（改善）** |
| hokkaido | 93.2% / vm下限 0.743 | **96.9%（悪化）/ vm下限 0.631（悪化）** |
| okinawa | 173.2% / vm下限 0.954 | **203.0%（悪化）/ vm下限 0.775（悪化）** |
| west | AC FAIL(バス数上限)/DC | 変化なし |

east は改善、**hokkaido と沖縄は悪化**。これは今日ずっと見てきた機構と同じで、
水増し太陽光が隠していた弱点の露出である。それでも 0.10 を採る根拠:
総量の誤差が |318%−100%|=218pt から |20%−100%|=80pt へ縮み、
夕方ピーク断面では太陽光出力はほぼゼロなので、10MW の値は
`balance_by_zone` の空間配分を歪めるだけの働きしかしていなかった。
**不満なら `--default-cap solar=10.0` で戻せる。**

**⚠ 是正で新しい限界が現れた**: 太陽光は実績ピーク比 **318%→20%** で、今度は**下回る**
（kyushu 2.6%・okinawa 0.6%）。OSM は小規模設備が大半で大規模事業所の `capacity_mw` が
入っていないため、件数×中央値では実績に届かない。**既定値では解けない** —
出典付き容量充填（1-C GEM 充填と同型）が次の一手。台帳#25 と監査に明記した。

**副産物の欠陥修正2件**:
- 特性テスト `test_hokkaido_dc_pins_the_effect_of_the_default_flip` が**設計どおり発火**した
  （90.2→128.8%）。原因を #25 と特定し、理由をコメントに書いてから固定値を更新
- **監査レポートの地の文が壊れた** — 10MW 前提の直書きで「既定値は 0 MW で **1 倍**ある」
  という文が出た。`_solar_section()` を足して**地の文も実測から書く**ようにし、
  過剰／不足／釣り合いの三通りを自動で出し分ける

gate: 1,273 passed / 0 failed / 3 skipped

## 2026-08-10(続3) — **Claude Opus 5** — 方針の問いに測定で答えた: 基幹モデルを製品へ

オーナーの問い「動くモデルから拡張するか、データの穴を埋めるか」に、意見でなく測定で答えた
（`model_resolution_strategy_2026-08-10.md`）。**答え=モデルの解像度をデータの解像度に合わせる。**

**① どの穴が埋まるかは既に分かっている**。基幹(≥154kV)には行き先がある
（keitouzu 154kV 757本/275kV 283本/500kV 142本・未解決端点697・食い違い92・
**飛騨信濃直流幹線(実在900MW HVDC)がbuiltに欠落**）。66/77kV には無い —
2026-06-11の66kVプログラムが逆推定3手法の交差検証で天井を証明済みで、
**08-10に降圧点の出典探索という別経路から同じ天井に到達した**（二度目の独立証明）。

**② 現行の全規模モデルは66kVで「動いて」いない**。夕方17時の断面で注入の45.9%が
太陽光ノードに載っている（17時の太陽光出力はゼロ。UC実給電なら0.0%・稼働8,235→359機）。
その太陽光の96.6%は既定10MWで埋めた合成容量。**もっともらしい潮流は実在しない太陽光を
薄く塗って成立していた** — ここから拡張すれば捏造ごと拡張する。

**③ 基幹モデルは既にあり全項目で優れている**（`build_backbone_net`・帳簿つき縮約・
load と gen の**両方**を集約）:

| 島 | 全規模 → **基幹** | 最大負荷率 | 超過潮流 | **偽電源** |
|---|---|---|---|---|
| hokkaido | 0→0本 | 86.0→**38.2%** | 0→0 | 32→**1** |
| east | 551→**113本** | 1,594.7→**402.0%** | 90,360→**43,388MW** | 291→**29** |
| west | 291→**53本** | 708.2→**553.7%** | 33,950→**16,452MW** | 490→**8** |
| okinawa | 9→3本 | 165.0→**138.1%** | 211→230 | 4→**0** |

**偽電源が east −90%・west −98%**。基幹モデルは全規模より正直でかつ物理的。
縮約の代償も帳簿に出る（断片から寄せた需要 east 4,310MW・west 7,337MW）。

**推奨**: 新しく作るのでなく位置づけを変える — ①基幹モデルを計算モードから製品へ昇格
②66/77kVは網でなく帳簿付き集約負荷＋データ資産 ③全規模モデルは回帰・比較用に残す。

**④ 副次的に #25 の障害が解けた**: 介入#26 を入れると太陽光0.10MWでも供給不足ゾーンが消える
（kansai 0.98→**1.47**・hokuriku の異常な5.00→2.24。全ゾーンが1.5〜3.9に収まる）。
**採用順序は #26 → #25**。

## 2026-08-10(続2) — **Claude Opus 5** — 大飯・高浜が出力3分の1: ゾーン帰属を座標で決めている

介入#25 の適用可否を測る過程で、**太陽光を正すと kansai だけ供給不足になる**
（容量 23,722MW / 需要×1.05 = 24,098MW = 比 0.98）ことに気づき追跡した。
太陽光とは無関係の、**出典を持っているのに捨てている**欠陥だった
（`zone_attribution_dispatch_2026-08-10.md`）。

`balance_by_zone` は発電機のゾーンを**繋がったバスの座標 zone だけ**で決める。結果:

| 発電所 | 出力 | 由来 | operator | 潮流上のゾーン |
|---|---|---|---|---|
| 大飯原子力 | 4,494MW | `kansai_plants.geojson` | 関西電力株式会社 | **hokuriku** |
| 高浜原子力 | 3,392MW | 同上 | 関西電力株式会社 | **hokuriku** |
| 舞鶴火力 | 1,800MW | 同上 | 関西電力 | **hokuriku** |

計 **9,686MW の関西電力設備が hokuriku の容量**として数えられている。hokuriku の
容量/需要比は 5.00 なので `scale=0.20` — **これらの機は出力 20% に抑えられる**
（kansai 自身は 78%）。ゾーン帰属を直すと大飯 899→**2,668MW**・高浜 679→**2,013MW**・
舞鶴 360→**1,068MW** と **3 倍**になる。

**モデルは答えを既に持っている** — 3件とも kansai の GeoJSON 由来で operator タグに
関西電力と書いてある。`balance_by_zone` が使っていないだけ。`capacity_bridge` の
`gen_zone_override` は存在するが **UC 経路しか使っていない**（銘板経路との非対称）。

**集計への影響は小さい**: west 過負荷 291→290・最大負荷率 708.2% 不変・
超過潮流 −5%。残る 853%/708% の説明にはならない。結論は二段構え:
**個別の発電所出力を引用してはいけない**（台帳「読み方」節に発電機出力も加えるべき）／
集計の過負荷診断への影響は小さい。

**介入#24 の未記録の副作用も 1,800MW ぶん含まれる**: `cap` は受電容量で繋ぎ先を選ぶので
**エリア境界を越えて繋ぎ替えうる**。west 8,775機中ゾーンが変わったのは 2 機だけだが、
うち舞鶴火力 1,800MW が kansai→hokuriku へ移った（大飯・高浜は `nearest` 時点で既に
hokuriku＝既存の問題）。台帳の介入#24 に副作用として追記した。

**未適用**。直すなら根拠は**モデル内の出典**（由来 GeoJSON のリージョンと operator タグ）に
限る — 外から帰属表を持ち込むと捏造になる。需要側の bus.zone は座標のまま。

## 2026-08-10(続) — **Claude Opus 5** — 66kV層の縮約を実測: 半端な縮約は悪化、等価回路なら改善

③の負の結果（降圧点は出典で埋められない）を受けて自分が出した推奨
「66kV 層を網として解くのをやめ帳簿付きで集約する」を、**提案で終わらせず測った**
（`whatif_lv_reduction_2026-08-10.{md,json}`）。**推奨は一度棄却され、直して復活した。**

**第1版（需要だけ寄せる）は悪化した**: east 551→**568** 本・west 291→**357** 本。
原因を測ると 66/77kV 層は**純輸入**（east 発電 25.4GW / 需要 46.6GW / 純 −21.2GW。
太陽光を 0.10MW に正すと −39.6GW）。つまり層には**発電も 25.4GW 載っている**ので、
需要だけ動かすと残された発電が地元で消費されなくなり横流れが**増える**。
等価回路としての縮約は需要と発電を一緒に寄せなければ成立しない — 半端な縮約は害。

**第2版（需要+発電）**:

| 島 | 現行 | 縮約(需要のみ) | **縮約(需要+発電)** |
|---|---|---|---|
| hokkaido | 0本 / 86.0% | 2本 / 106.3% | **0本 / 72.6%** |
| east | 551本 / 1,594.7% / 90,360MW | 568 / 1,667.7% / 101,238 | **409 / 2,018.8% / 68,211** |
| west | 291本 / 708.2% / 33,950MW | 357 / 715.8% / 38,785 | **247 / 704.0% / 29,457** |
| okinawa | 9本 / 165.0% | 5 / 159.5% | **5 / 159.5%** |

hokkaido・west・okinawa は改善。**east だけ本数と超過潮流は改善（−26%・−25%）するのに
最大負荷率が 2,018.8% へ悪化**する — 72.9GW の発電を 630 の降圧点へ集めるため新たな
一点集中が生まれる。清々しい勝ちでも負けでもない。

寄せられなかった需要（どの降圧点からも到達不能）は**捏造しないでそのまま開示**する:
east 198件/4,463MW・west 434件/9,766MW。これがモデルの穴の正味の大きさ。

**未適用**。採るなら縮約の帳簿（何をどこへ寄せたか）ごと介入台帳に登録する。

## 2026-08-10 — **Claude Opus 5** — 探索を4目的に直した: 「トレードオフではない」は**誤りだった**

08-09n で見つけた「合成 slack を数えていない」を目的関数に入れ、64 潮流を回し直した
（`repair_search_2026-08-10.{md,json}`）。**結論が変わった。**

| 島 | 現行を全目的で支配する構成（偽電源なし） | **偽電源を数える** | 採用済 cap の偽電源 |
|---|---:|---:|---|
| hokkaido | 2 件 | **0 件** | 30 → 32（+2） |
| east | 4 件 | **0 件** | 268 → 291（**+23**） |
| west | 3 件 | **1 件** | 406 → 490（**+84**） |
| okinawa | 0 件 | 0 件 | 4 → 4（±0） |

west に残る 1 件も `site/off/10MW`（過負荷 285→282・最大負荷率は 1,894% で同一）で
実質的な改善ではない。**08-09l の「ここはトレードオフではない／正直にすると物理的にも
正しくなる」は、目的関数が 1 つ欠けていたことによる artifact だった**（当該レポート冒頭に
訂正バナーを追記。本文は 3 目的当時の記録として残す）。

改善そのものは実在する — east 過負荷 603→303 本・最大 1,668→823%・捏造容量 −76GW。
訂正されたのは「**無償である**」という部分で、実際は **east +23・west +84 の偽電源**と
引き換えだった。採用済みの介入#24 を撤回する理由にはならないが、判断パッケージの
前提は書き換わる（`repair_adoption_decision_2026-08-09.md` §0 に追記済み）。

**教訓**: 3 目的の非劣解は「重み付けをしないから恣意が入らない」と書いたが、
**どの軸を目的に選ぶか自体が最大の恣意**だった。台帳に載っている捏造の種類
（#3 接続・#10 系 slack・#25 容量…）を機械的に目的へ写す仕組みが要る。
