...
- `build_pages_editor.py` に `--template` 追加(任意テンプレ派生)。**→ オーナー「merge」で本番反映済(`b6cd5a4`/`8c85623`)**: 提案版を本番昇格(`templates/editor.html`刷新→docs再生成・iframe?v=8、系統図サイドバー+一覧パネル+凡例を`style.css`に統合?v=32、提案ファイル/生成器撤去)。ドリフトテスト4 passed・本番headless error0・full pytest 1134 passed・:8088も刷新版(shim非混入)。**残(オーナー希望・context軽い時)**: SLD複母線(OSM忠実層=`substation_scope.py`の :8088 API化)。
