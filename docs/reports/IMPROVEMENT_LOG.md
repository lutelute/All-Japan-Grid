...
- **【発見・未修正】`ufls.restore_after_min: 60` が未実装**: 設定は「島が崩壊しなければ UFLS 遮断分は 1 時間で再送電」だが、計算は 3 時間戻さず段も再び動かない。津波後の全域崩壊(20 例中 18)はこの挙動に依存。直すと崩壊は減り 3 時間受電率は上がる方向。run_v7 以前の全結果が対象なので、直すかはオーナー判断
