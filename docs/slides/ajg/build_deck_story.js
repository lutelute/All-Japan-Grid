...
/* ===================== 4. 年表（5幕） ===================== */
{
  ...
  s.addText("6月上旬に5連リリース（v1.1.0〜v1.4.0）— 標準化と解ける化の集中期", {
    ...
    margin: 0 });
  foot(s, 5, "1:00");
  s.addNotes("【25分版はスキップ可①】この1枚が地図。以降のスライドは左上バッジ（幕色つき）で現在地を示す。斜体の問いの連鎖だけ読み上げる。");
}
...
/* ============ 19.8 動: 周波数が戻るさま(GIF) ============ */
{
  const s = pres.addSlide();
  s.background = { color: "0A0D1A" };
  s.addImage({ path: A + "freq_recovery.gif", x: 0, y: 0, w: 13.33, h: 7.5 });
  s.addNotes("周波数が戻るさま・2幕構成(COI制御層・2,400秒): 第1幕=最初の40秒をフルスクリーン超スロー(慣性→UFLS3段→GF底打ち、『この深さ47.5Hzは実系統では極めて稀=ブラックアウト一歩手前』の注記付き)。幕間バナーを挟んで第2幕=×約727早送りでLFC/EDCの約35分の登坂→50.00Hz→+0.1Hz行き過ぎ。オーナー指摘『慣性の40秒が見えない・登坂は早送りでよい』への応答。役割分担: 多機層も第10波LFC修正で回復するが長時間はCOI層が担当。1:00");
}
