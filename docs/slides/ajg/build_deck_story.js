...
/* ===================== 1. タイトル ===================== */
{
  ...
  s.addText("地図はある。モデルが無い。だから作った。", { x: 0.75, y: 6.1,
    ...
    margin: 0 });
  s.addNotes("30秒。右の図は本プロジェクトの実出力（全国の電圧クラス別送電網）。「地図はある、モデルが無い」だけ言って次へ。｜時間配分: フル約29分。25分枠は【25分版はスキップ可①〜④】の4枚(道筋地図・v1.3-1.4詳細・v1.7→詳細・AGC-Nブリッジ)を飛ばして約24分。");
}
...
/* ============ 3.5 動: OSM→モデルの組み上げ(GIF) ============ */
{
  ...
  s.addNotes("【手順の本体】OSM→モデルの6段階を1本のアニメで。①送電線の幾何(西→東に描画) ②電圧を7段補完で決定(色が付く) ③端点マッチングで結線(黄点=ジャンクション) ④変電所(白丸)と変圧器 ⑤発電所(★)を最寄り接続 ⑥県別需要×電圧重みで需要配分。全フレーム実データ。ここで手順の全体像を掴ませてから年表へ。1:30");
}
...
/* ===================== 4. 年表（5幕） ===================== */
{
  ...
  s.addText("6月上旬に5連リリース（v1.1.0〜v1.4.0）— 標準化と解ける化の集中期", {
    ...
    margin: 0 });
  foot(s, 5, "1:00");
  s.addNotes("この1枚が地図 — 各幕カードに赤の✗=つまずきを明記(『すんなり行っていない』の骨格)。以降のスライドは左上バッジ(幕色つき)で現在地を示す。赤✗→斜体の問いの順に読み上げる。※旧skip①指定を解除: 苦闘の年表は本編の核。");
}
...
/* ===================== 8. 第2幕: 解ける化と外部検証 ===================== */
{
  ...
  foot(s, 9, "1:30");
  s.addNotes("【25分版はスキップ可②】左下の衛星3連画像は v1.1 の比較タブ（衛星写真で位置を目視検証）。右図は自慢の1枚だが、次の幕でこれを自分で疑いにいく。");
}
...
/* ===================== 12. 第4幕: 観測と突合し、動かし続ける ===================== */
{
  ...
  foot(s, 13, "1:00");
  s.addNotes("【25分版はスキップ可③】静的データセットが「動く系統の観測器」になった枚。39–129 MWの一致幅は検証時点の実測。デモできるならここで flow_map を開く。");
}
...
/* ============ 14.5 動: SubSLDの読み方と全国展開(GIF) ============ */
{
  ...
  s.addNotes("SubSLDを動きで: 読み方3段(左=構内幾何/右=単線結線図/捏造ゼロ)→全国406所の機械生成を流す。第5幕の実物提示。詳細はSubSLD論文デッキへ。1:00");
}
...
/* ===================== 17. 動揺も解ける(AGC-N) ===================== */
{
  ...
  s.addText("下段=各機の相差角(±40°の動揺→減衰整定)。多機版はCOI簡約版よりやや深く沈む(−3.0 vs −2.5 Hz) — 定Z負荷とGF幅の実装差で、帳簿に開示。", {
    ...
    margin: 0 });
  foot(s, 17, "1:00");
  s.addNotes("【25分版はスキップ可④】「動揺がないのが違和感」への回答。UFLSの3段が1.6/2.0/2.7秒に入るのが拡大で見える。AGC100の話が出たら『これはAGC-54。Nは任意で、東なら数百機』。");
}
...
/* ============ 19.5 動: 東全域N-3とUFLS(GIF) ============ */
{
  ...
  s.addNotes("東全域の大擾乱実験(設計外N-3デモと明記): ピーク断面59.4GWで富津+東新潟+千葉10.9GWを同時脱落、900秒の全アーク。t≈5.3sでUFLS第1段(遮断5,935MW)が底48.50Hzを打ち、4局面 — ①慣性で急落 ②高速登坂(ガバナ+水力LFC、60sで49.19) ③停滞(60-180s、速い余力が尽きる) ④緩回復(15分で49.84、完全復帰はさらに数十分先)。可変速再生(×10/×30/×120バッジ付き)。オーナー指摘『リニアに戻るのは違和感、どこかで停滞するはず』への回答 — 停滞は実在し、900秒窓で見せる。1:15");
}
...
/* ============ 19.6 動: 全系統動揺(GIF) ============ */
{
  ...
  s.addNotes("東西動揺: 東183機(富津3,893MW)×西298機(川越3,990MW)、各系統最大機N-1を同時刻表示。東西はFC経由の直流連系のみで動揺は互いに伝わらない(独立実験と明記)。右=東西COIの比較 — 同規模の事故で同じ沈み方。オーナー指摘『沖縄は無関係・北海道は連系線の話になる』を受けて4島合成から東西2系統に絞った版。1:00");
}
...
/* ============ 19.7 動: 逆位相動揺(GIF) ============ */
{
  ...
  s.addNotes("逆位相動揺+綱引きの機構: 右下に新設した『綱の張り=群間位相角差Δδ』パネルと、地図上の電力矢印(位相の進んだ群→遅れた群、振れ幅で太さが変わる)で、なぜ逆位相になるかを説明 — 2つの慣性群が長い送電回廊(ばね)で繋がれた2重り系。西G(九州側61機)×東G(関西以東181機)、相関−0.999・周期2.4s。オーナー指摘『綱引きがわかりにくい』への応答。1:00");
}
...
/* ============ 19.8 動: 周波数が戻るさま(GIF) ============ */
{
  ...
  s.addNotes("周波数が戻るさま(東N-3・900秒に一本化): 前スライドと同じ事故・同じデータで、COI曲線が左から描かれ局面バナーが順に出る進行チャート — ①慣性急落 ②UFLS底打ち(48.49Hz) ③高速登坂 ④停滞 ⑤緩回復。終端で『15分で49.84Hz、50.00への完全復帰はさらに数十分先』を正直に。オーナー指摘『p25がわからない』への応答: 北海道・COI層2,400秒の別実験(旧版)は文脈が飛ぶため差し替え。旧freq_recovery.gifはリポジトリに残置。1:00");
}
...
/* ===================== 20. 24時間の断面 ===================== */
{
  ...
  s.addImage({ path: A + "fig_agc_24h.png", x: 2.5, y: 1.08, w: 8.35,
    h: 5.85 });
...
/* ============ 22.3 深掘り①: UC 24時間ディスパッチ(GIF) ============ */
{
  ...
  s.addNotes("深掘り① 計画(UC): fy2023r2・757機・9連系線UCの24時間ディスパッチを燃料別積み上げで掃引。需要線との一致・揚水/スピルも正直に描く。以降の潮流・動揺実験は全てこの解を土台にしている、という位置づけを一言。1:00");
}
...
/* ============ 22.5 深掘り②: ピーク断面ローディング(静止画) ============ */
{
  ...
  s.addNotes("深掘り② 流れ(潮流): UCピーク断面のAC解による送電線負荷率マップ。混んでいる回廊(赤・太)に名前ラベル。UCが検出した混雑費用+1.4%が『どこの線の話か』を地図で回収。1:00");
}
...
/* ============ 22.7 深掘り③: 系統の強さ(SCC/SCR・静止画) ============ */
{
  ...
  s.addNotes("深掘り③ 強さ(SCR): 全4島・全バスの短絡容量SCCマップ(古典近似: 運転中機のXd'背後V=1.0・負荷除外・実網Ybus — 近似は図中に開示)。青=強い/赤=弱い。SCR=SCC÷設備容量なので、赤い場所ほどインバータ電源の連系が難しい。『負荷があるのに弱い』全国ワーストに注記。逆位相動揺の連系剛性ともつながる話。1:00");
}
...
/* ============ 第6幕B2 動: 発散が育つ→収束する(GIF) ============ */
{
  ...
  s.addNotes("第6幕の核心を動きで: ニュートン反復を1回ずつ止めて電圧場を観察。介入前(#37/#38なし)=大阪都心と軽井沢・嬬恋から逸脱が育ち発散 / 介入後=(仮)12件+誤帰属275点の是正だけで5回収束。補足(2026-08-30再検証): #37v2(下流込み集計)実装後は#38なしでも収束する — 両介入は独立に正当(regionの正しさはAC以前の問題)だが、AC成立の必要条件としては#37v2が主。『反復を止めて観察する』手法そのものが見える。1:00");
}
...
/* ============ 第6幕D 動: 全史トレーラー(GIF) ============ */
{
  ...
  s.addNotes("全史トレーラー第2版(20フレーム・37秒): 従来の5幕(組み上げ→UC→3島AC→探偵編→点灯→東N-3)に、第6幕『綱引き』(西の逆位相2カット、電力矢印はΔδ実符号で反転)と第7幕『戻るさま』(東N-3の900秒アーク・4局面チャート)を追加。締めは『いまは、ある。』。0:45");
}
...
/* ===================== 第6幕E. 失敗も台帳に載せる ===================== */
{
  ...
  card(s, 0.9, 1.4, 5.7, 1.72, "✗ 介入#40 人口傾斜 — 退行して不採用",
    "国勢調査メッシュで需要配分を人口比に傾斜 → 手元データは全国人口の39%しか覆っておらず、被覆内だけ歪んで東西フルACがdc_fallbackに退行。採用せず既定OFF — 『負の結果』も理由つきでレジストリに残した", "C0392B", 11);
  card(s, 6.8, 1.4, 5.7, 1.72, "✗ 第10波 多機LFCバグ — 全成果物を作り直し",
    "二次制御のACEに周波数バイアスが欠落=実質無効(900秒たっても47Hz台に張り付き)。COI層と同形のB=K_SYS·負荷·f0を導入して修正 → 正典4島+N-3+GIF群を一括再生成", "C0392B", 11);
  card(s, 0.9, 3.32, 5.7, 1.72, "✗ 動揺GIFは3回作り直し",
    "『リニアに回復するのは違和感、停滞するはず』→ 停滞は実在(60-180s・速い水力余力の枯渇)、900秒窓で開示。『矢印が合っていない・位相シフト?』→ 速度と位相は90°ずれる(振り子と同じ) — 矛盾を説明に変えた", "1F4E79", 11);
  card(s, 6.8, 3.32, 5.7, 1.72, "○ だから台帳がある",
    "介入#1〜#40: 全件に理由・出典・可逆手順。(仮)は(仮)と明記し、オーナー裁定を記録。失敗を隠すと次の失敗が見えなくなる — 誠実さは態度ではなく、仕組み", "0F7B6C", 11);
  s.addText("つまずき → 観察 → 介入(台帳) → 検証 → だめなら戻す — このループが6ヶ月分、全部残っている", {
    x: 0.9, y: 5.35, w: 11.6, h: 0.4, fontFace: F, fontSize: 13, bold: true,
    color: INK, align: "center", margin: 0 });
  foot(s, 25, "0:50");
  s.addNotes("『意外とすんなり行っていない』の直球スライド。#40の負の結果・第10波LFCバグ・GIF作り直し3回を隠さず並べ、台帳の存在理由に落とす。0:50");
}
