...
/* ============ 19.5 動: 東全域N-3とUFLS(GIF) ============ */
{
  ...
  s.addNotes("東全域の大擾乱実験(設計外N-3デモと明記): ピーク断面59.4GWで富津+東新潟+千葉10.9GWを同時脱落。●発電機183機(色=各機の周波数)が全域で沈み、t≈5.1sでUFLS第1段がラッチ、■負荷が1割縮む(遮断5,935MW)。右=全機の周波数+COI+UFLS段ライン。北海道の単機N-1と違い、大きな系統は『複数落ちて初めて』UFLS域に入る — 規模と保護の関係を1枚で。1:00");
}
...
/* ============ 19.6 動: 全系統動揺(GIF) ============ */
{
  const s = pres.addSlide();
  s.background = { color: "0A0D1A" };
  s.addImage({ path: A + "national_swing.gif", x: 0, y: 0, w: 13.33, h: 7.5 });
  s.addNotes("全系統動揺: 4島542機、各島の最大機N-1を同時刻に揃えた合成表示(実験は独立・連系線は跨がないと明記)。右の4本のCOIで『慣性の差=沈み方の差』が一目 — 北海道だけUFLS域へ。1:00");
}
...
/* ============ 19.7 動: 逆位相動揺(GIF) ============ */
{
  ...
  s.addNotes("逆位相動揺(inter-areaモード): 西日本302機のCOI相対周波数で、九州側61機と関西側190機が相関−0.999・周期2.4秒の完全な綱引き。実網インピーダンスから測った連系剛性T_abが決める固有モード — 『インピーダンスを出してあるから見える』動き。1:00");
}
...
/* ============ 19.8 動: 周波数が戻るさま(GIF) ============ */
{
  ...
  s.addNotes("周波数が戻るさま(COI制御層・2,400秒): 慣性→UFLS3段→GF→LFC/EDCの登坂で約35分かけて50.00Hzへ。役割分担: 多機層も第10波LFCバイアス修正(ace=B・ω)で回復を再現(900s検証47.29→49.81Hz)、数十分の完全復帰はCOI制御層で描く。ピーク断面のN-3は余力枯渇で48.4Hzから戻れない(前スライドと対) — 『余力が回復を決める』。1:00");
}
...
/* ============ 第6幕D 動: 全史トレーラー(GIF) ============ */
{
  ...
  s.addNotes("全史を31秒で: 組み上げ→UC→3島AC・西だけ解けない→探偵編(犯人2人)→西点灯→東N-3擾乱(実シミュ)→フィナーレ『地図はあった。モデルが無かった。いまは、ある。』。島の点灯/減光は模式と明記、擾乱パートは実シミュデータ。総括の直前に流して余韻を作る。0:45");
}
