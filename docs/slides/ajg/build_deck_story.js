...
/* ============ 19.8 動: 周波数が戻るさま(GIF) ============ */
{
  ...
  s.addNotes("周波数が戻るさま(COI制御層・2,400秒): 慣性→UFLS3段→GF→LFC/EDCの登坂で約35分かけて50.00Hzへ。役割分担の開示: 多機層はLFC回復を再現しない(既知の限界)、回復は制御層。ピーク断面のN-3は余力枯渇で48.4Hzから戻れない(前スライドと対) — 『余力が回復を決める』。1:00");
}
