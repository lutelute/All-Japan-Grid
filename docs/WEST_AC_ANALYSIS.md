...
## 【解決追記 2026-06-10/11 — Fable 5】本書の結論はフルモデルでも覆った

本書が確定させた真因（154 kV未満下位網の悪条件 Ybus）への対処が累積した結果、
**地域別フルモデル（下位網込み）で全10地域が需要スケール無しに AC 収束**するに至った
（kansai vm 0.931、国家PF図 14,647 バス）。効いた要素（IMPROVEMENT_LOG 2026-06-10/11）:

1. **多電圧変電所**（クラス別バス+所内変圧器 — 異電圧線が変圧器に飲まれ線インピーダンス消失していた欠陥の解消）
2. **発電機 Q 制限 + AVR 設定値**（無限 VAr 源の卒業、enforce_q_lims で PV→PQ 切替）
3. **回廊電圧伝播**（無タグ枝の型付けで非標準・偽電圧の変圧器を削減）
4. **分岐塔への合成需要の除去**（bus type 'n'）
5. **周波数帰属の是正**（他周波数 TSO 設備・直流線の除外）
6. **境界注入**（県外流入を境界変電所へ）

本書の切り分け結果（Q 無関係・極短線副次的・hv≥154 で収束等）は当時のモデルに対して正しく、
上記の対処を導いた一次資料として保存する。なお全国ゾーナル（west 6 地域併合 ~9千バス）の AC は 2026-06-11 に
縫合+累積改善後+ゾーン別バランス(`balance_power_by_zone`)で再試行したが**依然 FAIL（DC=OK）**。
地域単体では全て収束するため、残差は併合規模の Ybus 条件と、kansai の CF 可用量不足が生む
数 GW の域間転送（縫合・合成回廊経由）と考えられる。明示的なゾーン間転送制御（DC-OPF 的
ディスパッチ）または権威パラメータ（Pillar 3）が次の手段。
