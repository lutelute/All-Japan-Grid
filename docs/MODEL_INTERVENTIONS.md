...
> **#28 追補(2026-08-15 非東京フォールバック)**: 他社は基幹系しか公表せず機械可読で解けないため、非東京の孤立A変電所はWikipedia/J-POWER(独立二次)へフォールバック(`docs/reports/isolated_fallback_wikipedia.json`)。名指し3件を出典付きverifiedで解決: **由良開閉所→紀北変換所**(阿南紀北直流幹線=DC連系・Wikipedia紀伊水道直流連系設備) / **上ノ国町→江差変電所**(Wikipedia上ノ国WF) / **大間町→上北**(大間幹線→むつ幹線・J-POWER aec.go.jp PDF)。⚠**モデル適用可否は繋ぎ先ノードの実在に依存**（繋ぎ先は変異名でモデルに実在＝江差変電所66kV/上北変電所500kV/紀北変換所）。**由良→紀北・上ノ国→江差は適用可**（TEPCO10件＋この2件で本系統外 2130→2112・18減・12孤立変電所が合流）。ただし紀北は非同期直流連系の変換所なのでAC枝でなくDC扱い要。**大間→上北は合流せず**＝大間町変電所が region 誤タグ（hokkaido・実体は青森=tohoku/east島）で島が違い枝が無視される→ region calibration が先（[[project_agj_island_classify]] の region誤タグ問題）。捏造回避=未解決8件は相手を空に。combined worklist=`docs/reports/tepco_connection_worklist.json`。
