...
| 21 | **重複ノードのdedup**（同一座標6桁+同一電圧の重複ノードを1バスに畳む） | 構造 | ★ | `build_island_net(dedup_nodes=True)`→`--dedup-nodes` | **除去であって接続追加でない**（座標はOSM幾何由来ゆえ完全一致=同一物理点。bbox重なりで同一OSMオブジェクトが別regionに二重抽出された分）。検証（`reports/west_fragmentation_rootcause_2026-07-09.md`）: west重複633グループの**飛騨変換所は同一osm_id=975217734が2抽出に存在**・**98.6%が同名一致**・線は一切足さず。効果=west断片化 2,531→544成分・主成分69→86%（east 532→312・87→90%）。⚠**断片化を直すがwest full ACの特効薬ではない**（dedup後もfactor≤0.6/CLI順序ではdc_fallback=AC収束は別課題）。周波数跨ぎ併合は起きない（build_island_netは同一島=同一周波数のみ処理） | bstatsの`n_dedup_merged`→JSON`dedup_nodes`。dedup OFFで従来と完全一致（merged=0・バス数不変=不変量） | `--dedup-nodes`省略（既定OFF）。**既定ON化はYbus指紋/正典全再生成を伴うためオーナー判断** |
