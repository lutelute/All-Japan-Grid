# 系統DB全面改修 実装計画 — OSM正・ボトムアップ・GridStitch
...
└─────────────────────────────────────────────┘
         ▼
[GridStitch] 編集→append専用ログ→機械適用→A/B検証→GitHub issue→採用→supplement統合→OSM ODbL還元
```
...
- GridStitch の `replay` でログ→build→島数をCI再実行し回帰検出。

---

## (5) 「DB改善ツール」命名 & 段階リリース

**決定名: GridStitch(グリッドスティッチ)／和名「系統DB改善ツール」**(オーナー決定 2026-06-14)。grid+stitch(接続を縫合する)。`ajgrid stitch` として動詞的にCLIへ収まる。衝突なし。

候補(採否):
- **GridStitch** ← 採用(接続を縫い合わせる含意・方針=物理接続を繋ぐに直結)
- GridMend(grid+mend=綻びを繕う)— 次点
...
|---|---|---|---|
| **P0 基盤(命名)** | GridStitch命名・`ajgrid stitch` サブコマンド・`docs/GRIDSTITCH.md`・README節。既存ループを束ねるのみ | 既存 `/api/verify` `/api/issue` `/api/built` の疎通。issue #28 再現 | 既存1103緑を不変で維持(機能追加のみ・モデル変更なし) |
...
| **P4 CIM/UI駆動** | `level2.py` emit を node-breaker オブジェクトから駆動。編集UIを node-breaker 表示に。GridStitch v0.2(cut/属性検証) | CIM往復(emit→検証)・編集→A/B→issue→採用→supplement→(将来)OSM changeset | CIM emit のスナップショットテスト。E8b cut機構テスト。全緑 |
...
- **[判断要] GridStitch のリリース範囲(v0.1の線引き)**: cut機構(E8b)・ρ/AC検証連動・認証/同時編集を v0.1 に含めず後送りする提案。最小ループ(編集→ログ→島数A/B→issue→採用)でリリースしてよいか。
...
**確定方針(A)**: OSMに書かれているもの(A)は正として尊重しバグ扱いしない。GridStitchは A を接続候補、B のみバグissueとして出す。判定器は島ごとに「OSM substation近接の有無・接続線の有無・degree」を算出して仕分ける。

### 追加分類C: 鉄道/別事業者網(P1b診断で判明 2026-06-15)
「線は届くのに島の変電所」を実測すると、相当数が**鉄道(JR)・別事業者網**だった(tokyo: 本線2本+届くのにdeg<2の島変電所18件 = **JR/鉄道7 + 非鉄道11**)。例: 沼津77kVヤード=「JR 西相模-沼津線」のみ・物井=1500V DC(鉄道)。これらは利用系統から**正当に分離**しており、busbarで繋ぐと**JR↔利用系統の捏造接続**になる(territory+frequency discipline 台帳99と整合)。

| 種別C | 判定 | 扱い |
|---|---|---|
| **C: 鉄道/別事業者** | 届く線が `name`/`operator` にJR・鉄道・別事業者 | **繋がない**(別網=正当な島)。除外しP1bの対象外 |

**P1bの真の対象(台帳125で再評価・縮小)**: 非鉄道島を「届く線が主系統の別変電所へ実到達するか」で再分類した結果——
- **真の束縛バグ(繋ぐべき)= tokyo 実質1〜2件**(宇都宮: 鹿沼/徳次郎/西宇都宮へ実feeder到達なのに島)。
- **OSM接続欠落= 16件**(NEC府中・椿本チエイン・清水…): feederが構内で完結し主系統に届かない=連系線がOSM未整備。

→ **ブランケット母線束縛は不採用**(16件のOSM欠落を捏造接続=「物理接続=真・捏造禁止」違反)。P1bは「OSM上feederが主系統に**実到達**している真の束縛バグだけを狭く繋ぐ」targeted fix に縮小。OSM欠落16件は**編集ツール(人間が実態で連系)+OSM還元**へ回す(=編集路線の正当性を裏付け)。鉄道/別事業者(分類C)は対象外。
...
→ これらは GridStitch v0.1 の「编集前バリデーション」として実装する。
