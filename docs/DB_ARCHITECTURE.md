...
## 7. 漸進移行ステップ（各ステップで 824 tests 緑を維持）

| Step | 内容 | 完了条件（テスト） |
|---|---|---|
| 0 | スキーマ migration v2 + feature_key 実装（`src/db/` 拡張） | 新テーブルの CRUD 単体テスト |
| 0.5 | `fetch_subdivided.py` の osm id 保存修正 | 取得 feature に osm_type/osm_id が付く |
| 1 | `scripts/db/ingest.py` — 現 GeoJSON を raw / enrichments に分解取込（`_` 接頭辞・`_name_source`・`_enriched_by` 規約で provenance 復元） | 全10地域 ingest 成功、件数一致（6,962 / 40,077 / 19,138） |
| 2 | `scripts/db/export.py` — DB → GeoJSON | **golden test**: 全10地域で現ファイルと意味等価（キー順・float 表現を正規化して比較）`tests/test_db_roundtrip.py` |
| 3 | enrich 6段の書き込み先を DB へ切替（1本ずつ）。GeoJSON は export 経由に | 各切替後に enrichment テスト 75 件緑 + roundtrip 緑 |
| 4 | P03 ingest 新設（generators.geojson も D層出力へ = 修理完了） | generators.geojson 688 件再生成一致 |
| 5（任意） | ローダの DB 直読み・audit の SQL 化・差分レポート CI | — |
