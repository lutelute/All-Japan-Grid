...
Several open datasets exist for international benchmarking. The IEEE test cases (14, 30, 118, 300-bus)~\cite{ieee_testcase} provide canonical benchmarks but lack geographic grounding. PyPSA-Eur~\cite{pypsa_eur} constructs a European transmission model, originally from ENTSO-E map data and, since 2025, natively from OSM~\cite{xiong2025} with route- and circuit-length agreement of $\rho=0.96$--$0.998$ against official statistics; PyPSA-Earth extends the workflow globally~\cite{pypsa_earth}. SimBench~\cite{simbench} provides fully synthetic German benchmark grids validated against regulator statistics. The U.S. Synthetic Grid project~\cite{birchfield2017} generates geographically realistic test networks. For Japan, no comparable open dataset exists at the transmission level.
...
Three independent checks quantify how far the extracted model can be
trusted. Each result is reported together with its scope limit in the
same sentence, because the limits are part of the result.

\subsection{Rank Correlation Against Utility-Published Flows}
TEPCO is, to our knowledge, the only Japanese operator publishing
per-line flow measurements. Scoring corridor usage of the extracted
model against these flows yields an interior Spearman rank correlation
of $\rho=0.721$ ($p\approx10^{-9}$, boundary-conditioned corridors
excluded) --- \textit{this is a capacity/topology proxy}: it measures
whether the model routes power through the same corridors, not that
flow magnitudes agree. The AC power flow solved on synthetic loads
correlates at $\rho\approx0.46$ (interior) and $0.60$ (trunk).
These figures are \textit{not} commensurable with the
$\rho=0.96$--$0.998$ reported for PyPSA-Eur~\cite{xiong2025}, which
scores route and circuit \textit{lengths} --- a different physical
quantity. Supporting structural scores: substation recall 86\,\%,
plant-attachment recall 55\,\%. All scores ship as JSON scorecards in
the repository.

\subsection{Voltage-Class Cross-Check (Kansai)}
Line voltage classes are independently cross-checked against Kansai
Transmission \& Distribution's official $\geq$154~kV trunk-line
disclosure: 37 of 38 matchable named lines agree (97\,\%). The limit:
only 38 of the 182 disclosed lines could be unambiguously matched to
OSM geometry, and agreement is asserted at voltage-class level only
(the utility's raw per-line values are not redistributed).

\subsection{Standards Round-Trip (CGMES)}
All 10 regional models export natively to IEC~61970 CIM/CGMES~2.4.15
(EQ+TP+SSH+SV+GL with boundary sets), pass strict schema validation
(10/10 \textsc{valid}, zero dangling references), and survive a
round-trip through the \textit{independent} implementation pandapower
\texttt{cim2pp} with bus-voltage agreement within $10^{-4}$~p.u.\ fixed
by a regression test. This is a self-consistency check against an
independent parser, not validation against external ground truth.

\section{Limitations}

This dataset is generated from crowdsourced OSM data and has inherent limitations: (1)~line impedances and transformer characteristics are synthetic estimates; (2)~endpoint matching may produce false connections ($\sim$2--3\% error rate); (3)~some operators, fuel types, and capacities remain unknown; (4)~no breaker-level data for contingency analysis; (5)~nodal demand uses voltage-class weights rather than measured profiles; the west-island AC solution relies on provisional infeed links whose routes and ratings are explicitly marked as unverified in the model and result ledgers (to be replaced if actual routes are published), and retains large balancing slack (13\% of demand at peak) with local low-voltage pockets; (6)~all AGC dynamic parameters (inertia constants, droop, governor/turbine lags, UFLS settings) are typical published values --- the frequency-response results are structural demonstrations, and generator granularity is plant-level, so trip scenarios upper-bound unit $N$-1; (7)~external validation is limited to TEPCO per-line flows and Kansai voltage classes (Section~\ref{sec:validation}) --- the remaining 8 areas have no comparable public ground truth. The dataset is suitable for research and education, but \textbf{not} for operational planning.

%% ========================================
\section{Conclusion}

This paper presented \textit{All-Japan-Grid}, an automated pipeline extracting Japan's 10-region, dual-frequency transmission grid from OSM data. To our knowledge, this is the first openly available dataset that \textit{combines} nationwide Japanese coverage (all 10 areas, 50/60~Hz), native CGMES exchange with independent round-trip validation, per-value provenance, and rank-correlation scoring against utility-published per-line flows; each ingredient individually --- OSM grid extraction~\cite{medjroubi2017,arderne2020,xiong2025} and external validation of open grid models~\cite{simbench} --- is established practice, and the novelty claim is strictly about the combination. The dataset (6,962 substations, 40,077 lines, 19,138 plants) with 20 power flow solvers and MILP unit commitment provides a research-ready baseline. A national UC test demonstrated optimality in under 10~s with 1.4\% congestion cost, and the operations chain now extends through power-flow verification to multi-area AGC: the same UC solution supplies inertia and regulation headroom to an IEEJ-AGC30-based load-frequency simulation whose inter-area coupling is measured from the extracted network. Future work includes supplementation with official OCCTO data for engineering-grade accuracy.
...
\bibitem{xiong2025}
B.~Xiong, D.~Fioriti, F.~Neumann, I.~Riepin, and T.~Brown, ``Modelling the high-voltage grid using open data for Europe and beyond,'' \textit{Sci. Data}, vol.~12, 2025, doi:10.1038/s41597-025-04550-7.

\bibitem{pypsa_earth}
M.~Parzen \textit{et al.}, ``PyPSA-Earth. A new global open energy system optimization model demonstrated in Africa,'' \textit{Appl. Energy}, vol.~341, art.~121096, 2023.

\bibitem{simbench}
S.~Meinecke \textit{et al.}, ``SimBench---A benchmark dataset of electric power systems to compare innovative solutions based on power flow analysis,'' \textit{Energies}, vol.~13, no.~12, art.~3290, 2020.
