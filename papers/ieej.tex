...
(hot/warm/cold), battery storage SOC dynamics, and nine inter-regional
interconnection constraints dispatches 646 generators (636 thermal +
10 regional batteries totalling 3.5~GW/13.8~GWh) over 24 hours,
using OCCTO-calibrated renewable capacities (59~GW solar, 11~GW wind);
Kyushu noon solar reaches 83\% of its regional peak demand,
revealing curtailment-driven battery dispatch as a key system need.
Transient N-1--N-10 cascade analysis (30,911 cases) confirms full stability
($\leq$2.66$^\circ$ maximum rotor angle separation), demonstrating strong network connectivity.
\end{abstract}
...
\begin{enumerate}
  ...
  \item 646機・OCCTO参照再エネ（太陽光59~GW・風力11~GW）・蓄電池3.5~GWを含むMILP UCの求解
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_layer_combined.png}
  \caption{GeoJSONレイヤ別展開：
  (a)~送電線ネットワーク（40,077本，電圧クラス別色分け），
  (b)~変電所分布（6,962箇所，電圧クラス別），
  (c)~発電所（赤△：大型集中電源[原子力・火力・大型水力等, 1,128箇所]，
  黄点：分散型再エネ[太陽光・風力等, 18,010箇所]）}
  \label{fig:layers}
\end{figure*}
...
\subsection{646機MILP求解の技術的背景}

646機（熱電源636機・地域別蓄電池10機）・24時間のMILPは二値変数$u_{g,t}$が
$636 \times 24 = 15{,}264$個，連続変数（$p,v,w,f$および蓄電池SOC変数）を含め
総変数約65{,}000個に達し，一般的な商用ソルバで
...
発電所分類（大型集中電源1,128箇所 vs 分散型再エネ18,010箇所）は
自由化後の再エネ大量導入を明確に反映する．
分散型再エネのうち約94\%（17,000箇所超）が太陽光発電であり，
関東・東海・九州北部の日射良好地帯に面的展開していることが確認できる．
...
646機（熱電源636機＋地域別蓄電池10機）・9連系線・蓄電池SOCダイナミクスを含む
全制約で最適解を得た（Table~\ref{tab:uc_res}，Fig.~\ref{fig:uc3state}）．
N-$x$過渡安定解析については，全国500+275~kV系統の実GeoJSONトポロジから
Kron縮約により発電機バス100点の等価ネットワークを構築し，
DC潮流による事前平衡点を初期値として古典機モデルを適用する
（Table~\ref{tab:nx}，Fig.~\ref{fig:dynamics}）．
太陽光はOCCTO 2023年度統計ベースの地域別設備容量（全国計59~GW）に
地域日射倍率（北海道$\times0.83$〜沖縄$\times1.13$）を乗じたCFプロファイルを適用した．
その結果，九州では正午の太陽光発電量がピーク需要の83\%に達し，
蓄電池が正午充電・夕刻放電サイクルを自動選択した．
UCモデルの連系線フローはDC線形近似\cite{stott2009dc}を採用し，
旧OSMモデル（OSM由来容量のみ）では混雑コスト+1.40\%（¥107~M/日）を確認していたが，
OCCTO参照モデルでは再エネ大量導入により净需要が減少し総費用が削減された．
原子力・石炭が基底，LNGがミドル，石油がピーク時稼働という
merit order構造が各地域で自動再現された．

\begin{table}[t]
  \caption{全国UC最適化結果（24時間，9連系線制約，蓄電池SOC付き）}
  \label{tab:uc_res}
  \centering\small
  \begin{tabular}{lrrr}
    \toprule
    モデル & コスト（億/日） & 時間 (s) & 機数 \\
    \midrule
    旧OSMモデル・単一起動コスト & 76.51 & 9 & 783 \\
    旧OSMモデル・3状態起動コスト & 84.00 & 39 & 783 \\
    \textbf{OCCTO参照・3状態起動コスト} & \textbf{68.00} & \textbf{38} & \textbf{646} \\
    \midrule
    連系線混雑コスト（旧モデル） & \multicolumn{3}{l}{$+$1.40\%（¥107M/日）} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure*}[t]
  ...
  \caption{10地域別UC 24時間日間プロファイル（OCCTO参照容量：太陽光59~GW・風力11~GW・蓄電池3.5~GW）——
  地域別日射倍率を適用した太陽光CF（九州$\times1.10$等）と地域需要（OCCTOピーク実績）を使用；
  九州正午に太陽光がピーク需要の83\%に到達し蓄電池がcurtailment緩和に自動投入される}
  \label{fig:uc3state}
\end{figure*}
...
N-1（全100バス）およびN-2〜N-10（上位15機の$\binom{15}{k}$全組合せ）について
過渡安定解析を実施した（安定判定：$\max_{ij}|\delta_i-\delta_j|<\pi$）．結果をTable~\ref{tab:nx}に示す．

\begin{table}[t]
  \caption{N-$x$過渡安定解析結果（500+275~kV, Kron縮約，上位15機カスケード）}
  \label{tab:nx}
  \centering\small
  \begin{tabular}{lrrc}
    \toprule
    擾乱 & 件数 & 不安定 & 最大角度差 \\
    \midrule
    N-1（全100バス） & 100 & 0 & $2.64^\circ$ \\
    N-2〜N-10（$\sum\binom{15}{k}$） & 30{,}811 & 0 & $\leq2.66^\circ$ \\
    \midrule
    \textbf{合計} & \textbf{30{,}911} & \textbf{0} & $\leq2.66^\circ$ \\
    \bottomrule
  \end{tabular}\\[2pt]
  {\scriptsize N-$x$次数増大でも最大角度差は2.64$^\circ$→2.66$^\circ$と微増に留まり，高い冗長性を示す}
\end{table}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_nx_proper.png}
  \caption{全国500+275~kV系統 N-$x$過渡安定解析（実GeoJSONトポロジ，Kron縮約，DC潮流初期値）——
  (a)~N-1最大角度偏差の地理分布（発電機バス100点）；
  (b)~最悪N-1後のロータ角時空間ヒートマップ（$\Delta\delta_i(t)$）；
  (c)~最大変動5機のスイング曲線（全99機で収束振動，最大2.64$^\circ$）}
  \label{fig:dynamics}
\end{figure*}

% N-10 cascade figure (Fig.~\ref{fig:nxcascade}) is available in the supplementary material.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    ...
    UC（旧OSMモデル） & 783機×24h & 39~s \\
    UC（OCCTO参照モデル） & 646機×24h & 38~s \\
    N-1（全100バス） & 100ケース & 250~s \\
    N-1〜N-10（連鎖脱落） & 30{,}911ケース & $\sim$30~min \\
...
本研究で達成した646機（OCCTO参照再エネ・蓄電池込み）・9連系線・24時間のUCは，
著者らの知る限り日本の送電系統を対象とした公開研究中で最大規模であり，
HiGHSソルバによる38秒での求解は実用的な日次計画への応用可能性を示す．
また，旧OSMモデルで確認した全9連系線飽和・混雑コスト+1.40\%（¥107~M/日）は
東西周波数変換断面が全国最適化のボトルネックであることを示す定量的根拠であり，
OCCTO参照モデル（太陽光59~GW・地域別ノード需給バランス制約）では再エネ純需要削減により総費用が約19\%低減（¥84億→¥68億/日）した．
...
なお4次同期機（Park方程式），AVR（IEEE ST1A），GFLインバータ，
短絡容量・小信号・継続潮流の解析フレームワーク（\texttt{src/dynamics/}）を実装済みであり，
精度の高い動的パラメータが得られた段階での本格的な動的解析への接続基盤が整っている．
...
なお発電機は立地座標で地域帰属しており（電力会社所有帰属とは意図的に異なる），
地理的電源分布（どこから電力が送られているか）の把握が目的である．
現状の主な限界は，インピーダンス合成推定（実測値非），
誤接続率2--3\%，遮断器情報なし（電気的N-1不可），
発電機動特性・負荷モデル未実装であり，
さらに連系線で統合した広域一括AC潮流は下位網（154~kV未満）の規模と
悪条件のため収束困難であり地域分割解析が現実的である（地域別ACは全10地域で収束）．
...
全10地域AC潮流収束，OCCTO参照モデル646機MILP UC（¥68億/日，38秒求解），
九州正午太陽光83\%のcurtailment-driven蓄電池投入，
N-1〜N-10（上位15機，$\binom{15}{k}$全組合せ）30{,}911ケース全安定（最大$2.66^\circ$）を確認した．
