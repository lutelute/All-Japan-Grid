export const meta = {
  name: 'gem-capacity-adjudicate-verify',
  description: '1-C GEM容量充填: 曖昧1901件の同定裁定+AUTO134の敵対的検証+未マッチ103の捜索',
  phases: [
    { title: 'Adjudicate', detail: '曖昧ペア77バッチの同定裁定+バッチ毎スポット反証' },
    { title: 'VerifyAuto', detail: 'AUTOマッチ(major全49+solar標本30)の敵対的検証' },
    { title: 'Search', detail: '未マッチmajor 103件のgem.wiki捜索+同定検証' },
  ],
}
...
// ---- Phase 1: 裁定(pipeline: 裁定 → バッチ内accept上位のスポット反証) ----
phase('Adjudicate')
const adjBatches = Array.from({ length: A.nAdjBatches }, (_, i) => i)
const adj = await pipeline(
  adjBatches,
  (i) => agent(
    `あなたは日本の発電所データの同定裁定者です。九州/沖縄のOSM由来発電所(容量未知)と Global Energy Monitor wiki のページ候補の突合を裁定します。\n` +
    `ファイル ${S}/wf_adj/batch_${pad(i)}.json を Read してください。各要素 = {plant: {key,name,fuel,lat,lon}, note?, candidates: [{title, ja_name, loc, category, lat, lon, url, operating_units, operating_total_mw, other_status, distance_m, name_eq}]}。\n` +
  ...
  (res, i) => {
    ...
    return agent(
      ...
      `手順: 各 match_title の gem.wiki ページ (https://www.gem.wiki/<title の空白を_に>) を WebFetch し、そのページが本当に対象plant(ファイル ${S}/wf_adj/batch_${pad(i)}.json 内の該当要素)と同一施設かを、和名・所在地・座標・容量規模で点検。同定が誤りらしければ refuted=true。ページ取得に失敗したら refuted=false とし reason に fetch-failed と書く。\n` +
...
const autoVer = await parallel(verItems.map(k => () => agent(
...
log(`裁定: ${verdicts.length}件中 accept=${nAccept} / スポット反証=${spots.filter(s => s.refuted).length}/${spots.length}`)
