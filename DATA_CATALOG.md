...
> **重要 — 件数の正本はこのカタログ（=実測値）**
> README / WHITEPAPER の表記には食い違いがある。実ファイルを数えた結果、変電所の**合計は 6,962**。
> README 冒頭の地域別表（L73-85）の **Total 行だけが「7,962」と誤記**されている（地域別の各行 471+901+…+59 を足すと正しく 6,962 になるので、合計欄の単純な書き間違い）。
> 一方、同じ README の品質監査表（L160）と WHITEPAPER（L175）は **6,962** で実測と一致。送電線 40,077・発電所 19,138 は全資料で一致。
> → **正: 変電所 6,962 / 送電線 40,077 / 発電所 19,138**。README 冒頭表の 7,962 は要修正。
...
| 指標 | 実測（本カタログ正本）| README 冒頭表 L85 | README 監査表 L160-162 | WHITEPAPER L175 |
|---|---:|---:|---:|---:|
| 変電所 | **6,962** | 7,962 ❌ | 6,962 ✅ | 6,962 ✅ |
| 送電線 | **40,077** | 40,077 ✅ | 40,077 ✅ | 40,077 ✅ |
| 発電所 | **19,138** | 19,138 ✅ | 19,138 ✅ | 19,138 ✅ |

→ **README 冒頭の地域別表の Total 行「7,962」のみ実測と不一致**（地域別各行の合計は 6,962 で正しい＝合計欄の誤記）。修正推奨。
