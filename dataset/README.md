...
- **「潮流が収束した」＝「正しく解けた」ではありません。** 配布 MATPOWER ケースは建造断面の
  スナップショットで、多成分の島（例: 北海道は 9 成分・各成分 1 slack）では発電スケジュールと
  負荷が時間断面として整合しておらず、単純な `runpf` では損失が負になる等の不整合が出ます。
  潮流の挙動確認は単一成分の**沖縄**で、大規模島の需給整合は **UC → 潮流連成**で行ってください。
